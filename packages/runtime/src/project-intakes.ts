...
export const projectIntakeReferenceSchema = z.string().regex(/^[a-z][a-z0-9_-]{2,31}:[A-Za-z0-9_-]{8,160}$/);
...
export const projectIntakeDraftSchema = z.strictObject({ schemaVersion: z.literal(1), expectedRevision: z.number().int().positive(), idea: z.string().max(20_000), workspaceReference: projectIntakeReferenceSchema.nullable(), attachmentReferences: z.array(projectIntakeReferenceSchema).max(100) });
