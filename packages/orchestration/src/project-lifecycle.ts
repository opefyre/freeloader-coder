...
export const ownerQuestionSchema = z.strictObject({
  ...
  recommendedOptionId: z.string().trim().min(1).max(80).nullable().optional(),
  affectedArtifacts: z
    .array(z.string().trim().min(1).max(160))
    .max(20)
    .optional(),
  dependsOnFindingIds: z
    .array(z.string().trim().min(1).max(160))
    .max(30)
    .optional(),
});
...
export type ProjectLifecycleRecord = z.infer<
  typeof projectLifecycleRecordSchema
>;
  ...
  switch (event.type) {
    ...
    case "context_completed": {
      ...
      if (event.artifact.kind !== "context" || !event.artifact.qaPassed) {
        throw new Error(
          "Context review requires a QA-passed CONTEXT.md artifact.",
        );
      }
      patch = {
    ...
    case "scope_assessed":
      ...
      if (event.assessment.classification === "small_change") {
        patch = {
          ...
          blockedReason:
            "The request is below the configured major-work threshold.",
        };
      } else if (
        event.assessment.classification === "unclear" ||
        event.assessment.confidence < 0.75
      ) {
        const questions = (event.questions ?? []).map((question) =>
          ownerQuestionSchema.parse(question),
        );
        if (questions.length === 0)
          throw new Error(
            "Unclear scope requires a selectable owner clarification.",
          );
        patch = {
          stage: "clarification",
          assessment: event.assessment,
          questions,
          blockedReason: "Major-work scope is not yet certain.",
        };
      } else {
        patch = {
    ...
    case "design_completed":
      ...
      if (event.artifact.kind !== "solution" || !event.artifact.qaPassed) {
        throw new Error(
          "Owner review requires a QA-passed SOLUTION.md artifact.",
        );
      }
      patch = {
    ...
    case "design_declined":
      ...
      patch = {
        ...
        blockedReason: "The owner declined the proposed solution.",
      };
      break;
    case "design_revision_requested":
      requireStage(record, "awaiting_design_approval");
      requireArtifact(record, "solution", event.artifactDigest);
      if (event.feedback.trim().length < 3)
        throw new Error("A solution revision needs specific owner feedback.");
      patch = {
        stage: "solution_design",
        designApproval: null,
        designFeedback: [
    ...
    case "backlog_completed":
      ...
      if (event.artifact.kind !== "backlog")
        throw new Error("Backlog design must produce BACKLOG.md.");
      if (!event.artifact.qaPassed)
        throw new Error(
          "Jira creation requires an independently reviewed backlog.",
        );
      patch = {
        stage: "backlog_qa",
        artifacts: upsertArtifact(record.artifacts, event.artifact),
    ...
    case "backlog_qa_passed": {
      ...
      if (!backlog.qaPassed || event.jiraEpicId.trim().length === 0)
        throw new Error(
          "Delivery requires a QA-passed backlog and its Jira epic.",
        );
    ...
    case "owner_input_required":
      if (["complete", "cancelled"].includes(record.stage))
        throw new Error("Terminal projects cannot request owner input.");
    ...
    case "reopen":
      if (
        !(["complete", "cancelled"] as const).includes(
          record.stage as "complete" | "cancelled",
        )
      )
        throw new Error("Only a terminal project can be reopened.");
      if (event.mission.trim().length < 3 || event.reason.trim().length < 3)
        throw new Error("Reopening requires a mission and a reason.");
      patch = {
        stage: "context_review",
        mission: event.mission.trim(),
        assessment: null,
        questions: [],
        answers: [],
        designApproval: null,
...
function validateAnswers(
  questions: readonly OwnerQuestion[],
  rawAnswers: readonly OwnerAnswer[],
): OwnerAnswer[] {
  const answers = rawAnswers.map((answer) => ownerAnswerSchema.parse(answer));
  if (answers.length !== questions.length)
    throw new Error("Every blocking clarification must be answered.");
  const seen = new Set<string>();
  for (const answer of answers) {
    if (seen.has(answer.questionId))
      throw new Error("A clarification cannot be answered more than once.");
    ...
    if (!question)
      throw new Error(
        "Clarification answer does not match an active question.",
      );
...
function requireStage(
  ...
  stage: ProjectLifecycleRecord["stage"],
): void {
  if (record.stage !== stage)
    throw new Error(`Event requires ${stage}; project is ${record.stage}.`);
}

function requireArtifact(
  record: ProjectLifecycleRecord,
  kind: "solution" | "backlog",
  artifactDigest: string,
) {
  const artifact = record.artifacts.find(
    (candidate) =>
      candidate.kind === kind && candidate.digest === artifactDigest,
  );
  if (!artifact)
    throw new Error(`The approved ${kind} artifact revision was not found.`);
  return artifact;
}
...
function inferResumeStage(
  record: ProjectLifecycleRecord,
): ProjectLifecycleRecord["stage"] {
  if (record.designApproval?.decision === "approved" && !record.jiraEpicId)
    return "backlog_design";
  if (record.jiraEpicId) return "delivery";
  if (record.artifacts.some((artifact) => artifact.kind === "solution"))
    return "awaiting_design_approval";
  if (record.artifacts.some((artifact) => artifact.kind === "context"))
