...
const command = await readFile(
  "apps/studio/src/components/projects/project-stage-command.tsx",
  "utf8",
);
const app = await readFile("apps/studio/src/App.tsx", "utf8");
const portfolio = await readFile(
  "apps/studio/src/components/projects/project-portfolio.tsx",
  "utf8",
);
...
test("project detail presents one bounded owner command with consequences and recovery", () => {
  ...
  for (const contract of [
    ...
    assert.equal(
      ...
      true,
      `missing command contract: ${contract}`,
    );
  }
...
test("project cards can shrink to the mobile viewport without clipping progress evidence", () => {
  assert.match(portfolio, /grid min-w-0 gap-2 sm:grid-cols-2/);
  assert.match(portfolio, /min-h-36 min-w-0 w-full/);
  assert.match(portfolio, /mt-auto min-w-0 w-full pt-4/);
});
