  ...
  recentEvents: [
    {
      ...
      detail: "token=super-secret /Users/private/repo failed.",
      observedAt: now - 100,
      projectId,
      requestId,
      providerId: "groq",
    ...
    {
      id: "event_1123456789abcdef",
      kind: "project",
      state: "warning",
      title: "Project warning",
      detail: "A deterministic scan needs review.",
      observedAt: now - 200,
      projectId,
      requestId: null,
      providerId: null,
...
test("decision aggregation maps canonical blockers, prioritizes, deduplicates, and summarizes", () => {
  ...
  assert.deepEqual(
    [...new Set(snapshot.items.map((item) => item.category))].sort(),
    ["approval", "failure", "project", "provider", "recovery"],
  );
...
test("decision filters are conjunctive and unknown healthy observations create no work", () => {
  const filtered = buildDecisionSnapshot({
    live,
    autonomy,
    query: {
      range: "24h",
  ...
  const none = buildDecisionSnapshot({
    live,
    autonomy,
    query: { range: "24h", search: "healthy" },
    now,
  });
...
test("decision privacy, authority, aging, cost, and bounded-retention rules are enforced", () => {
  ...
  assert.ok(
    snapshot.items.every(
      (item) => item.authorityBoundary && item.evidence.length > 0,
    ),
  );
  assert.equal(snapshot.retention.maximumItems, 250);
  assert.ok(
    snapshot.items.every((item) => item.reference.path.startsWith("/")),
  );
});
...
test("unanswered lifecycle clarifications appear as project-scoped owner actions", () => {
  const lifecycle = {
    ...
    revision: 2,
    mission: "Build a portal.",
    assessment: null,
    questions: [
      {
        id: "question_0123456789abcdef",
        prompt: "Who can sign up?",
        whyItMatters: "Identity architecture changes.",
        options: [
          { id: "invite", label: "Invite", consequence: "Admins invite." },
  ...
  const snapshot = buildDecisionSnapshot({
    live: { ...live, providers: [], recentEvents: [] },
    autonomy: { ...autonomy, recommendations: [], leases: [] },
...
test("reviewed solutions appear as digest-backed owner approvals", () => {
  const lifecycle = {
    schemaVersion: 1 as const,
    projectId,
    stage: "awaiting_design_approval" as const,
    revision: 5,
    mission: "Build a portal.",
    assessment: null,
    questions: [],
    ...
    artifacts: [
      {
        ...
        projectRelativePath: ".pipeline/SOLUTION.md" as const,
        digest: "b".repeat(64),
        revision: 1,
        createdAt: now - 30,
        citations: ["local://CONTEXT.md"],
        reviewerIds: ["product-reviewer", "technical-reviewer"],
        qaPassed: true,
      },
    ],
    designApproval: null,
  ...
  assert.match(
    snapshot.items[0]?.evidence.join(" ") ?? "",
...
test("delivery execution failures replace superseded request prompts with a current owner action", () => {
  ...
  const execution: ProjectExecutionRecord = {
    ...
    tasks: [
      {
        id: "plan_0000000000000004",
        jiraIssueKey: "PIPE-22",
        ...
        assignment: {
          providerId: "gemini",
          modelId: "free",
          deviceId: "provider:gemini",
          selectedAt: now - 20,
          reasons: ["Free route selected."],
        },
        lease: null,
        implementationEvidence: ["d".repeat(64)],
        validations: [],
        reviews: [],
        commitDigest: null,
        integrationDigest: null,
        failureClass: null,
        safeMessage: "Validation environment needs repair.",
        updatedAt: now - 5,
      },
    ],
  };
  const staleAutonomy = {
    ...autonomy,
    recommendations: [
      { ...autonomy.recommendations[0]!, boundary: "approve_request" as const },
    ],
    leases: [],
  };
  const snapshot = buildDecisionSnapshot({
    live: { ...live, providers: [], recentEvents: [] },
    autonomy: staleAutonomy,
    lifecycles: [lifecycle],
  ...
  assert.equal(
    ...
    "PIPE-22 · Execution environment needs repair",
  );
  assert.equal(snapshot.items[0]?.priority, "high");
  assert.equal(snapshot.items[0]?.projectId, projectId);
  assert.equal(
    snapshot.items[0]?.reference.path,
    `/projects?project=${projectId}`,
  );
  assert.match(
    snapshot.items[0]?.nextAction ?? "",
    /repair the local project environment/i,
  );
  assert.match(
    snapshot.items[0]?.evidence.join(" ") ?? "",
...
test("missing live proof appears once with an exact owner recovery action", () => {
  const execution: ProjectExecutionRecord = {
    ...
    tasks: [
      {
        id: "plan_0000000000000004",
        jiraIssueKey: "PIPE-44",
        title: "Verify owner journey",
        dependsOn: [],
        allowedFiles: ["src/app.tsx"],
        validationProfiles: ["build", "visual"],
        uiChanged: true,
        requiredCapabilities: ["chat"],
        privacyClass: "source_code",
        status: "needs_user",
  ...
  assert.equal(
    snapshot.items[0]?.title,
    "PIPE-44 · Live journey proof required",
  );
  assert.match(
    snapshot.items[0]?.nextAction ?? "",
    /exact integrated revision/i,
  );
  assert.match(
    snapshot.items[0]?.evidence.join(" ") ?? "",
...
test("terminal lifecycles suppress obsolete execution and live-event owner actions", () => {
  const lifecycle = {
    schemaVersion: 1 as const,
    projectId,
    ...
    tasks: [
      {
        ...
        safeMessage: "Historical execution needs repair.",
        updatedAt: now - 5,
      },
    ],
  };
  const snapshot = buildDecisionSnapshot({
    live: {
      ...live,
      providers: [],
      recentEvents: [{ ...live.recentEvents[1]!, observedAt: now - 2 }],
    },
    autonomy: { ...autonomy, recommendations: [], leases: [] },
    lifecycles: [lifecycle],
    executions: [execution],
  ...
  assert.equal(snapshot.summary.blockedProjects, 0);
});
