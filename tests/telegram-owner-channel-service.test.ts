...
test("Telegram owner channel signs, authorizes, consumes, and rejects replayed solution decisions", async () => {
  ...
  try {
    ...
    const service = new TelegramOwnerChannelService(
      ...
      { read: async () => JSON.stringify({ schemaVersion: 1, botToken: token, chatId }) },
      async (input, init) => {
        ...
        if (method === "getUpdates") {
          poll += 1;
          return Response.json({ ok: true, result: poll === 1 ? [] : [{ update_id: poll, callback_query: { id: `callback-${poll}`, data: callbackData, message: { chat: { id: Number(chatId) } } } }] });
        }
...
test("Telegram owner channel rejects a valid signed response from a different chat", async () => {
  ...
  try {
    ...
    const service = new TelegramOwnerChannelService(root, { list: async () => projects }, { list: async () => [lifecycle], get: async () => lifecycle, answer: async () => lifecycle, decideSolution: async () => { decisions += 1; return lifecycle; } }, { read: async () => JSON.stringify({ schemaVersion: 1, botToken: token, chatId }) }, async (input, init) => {
      ...
      if (method === "getUpdates") { poll += 1; return Response.json({ ok: true, result: poll === 1 ? [] : [{ update_id: 2, callback_query: { id: "foreign", data: callbackData, message: { chat: { id: -999999999 } } } }] }); }
      return Response.json({ ok: true, result: true });
    }, () => 1_000);
    await service.synchronize(); await service.synchronize();
