...
test("execution model admits an exact free assignment, records it, and replays durable output", async () => {
  ...
  try {
    ...
    const repository = {
      list: async () => [stored],
      read: async (id: string) => (id === stored.id ? stored : null),
    };
    const model = new FreeProviderExecutionModel(
      root,
      repository,
      { read: async () => "safe-test-credential" },
      {
        adapter: (providerId) =>
          ({
            manifest: { providerId },
    ...
    const input = {
      ...
      assignment: {
        ...
        deviceId: `provider:${stored.id}`,
      },
      role: "implementer" as const,
      permit,
      system: "Return JSON.",
      instruction: "Propose bounded changes.",
      sources: [{ name: "src/feature.ts", content: "export const value = 1;" }],
...
test("execution model sends Cohere a portable structured-output schema without weakening the canonical contract", async () => {
  ...
  try {
      ...
      properties: {
        operations: {
          ...
          items: {
            type: "object",
            properties: {
              kind: { enum: ["create", "replace"], type: "string" },
              path: { type: "string" },
            },
          },
        },
    ...
    assert.equal(
      canonicalSchema.properties.operations.items.properties.path.minLength,
      1,
    );
  } finally {
    await rm(root, { recursive: true, force: true });
...
test("execution model automatically refreshes expired free-provider evidence before assignment", async () => {
  ...
  try {
    let stored = connection({
      state: "stale",
      quota: { ...connection().quota, expiresAt: now - 1 },
    });
    let refreshes = 0;
    const repository = {
      list: async () => [stored],
...
test("execution model cools down failed stale-provider probes before trying again", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "execution-model-refresh-cooldown-"),
  );
  try {
    let clock = now;
    const stored = connection({
    ...
    assert.equal((await model.candidates(permit, "reviewer")).length, 0);
    assert.equal((await model.candidates(permit, "reviewer")).length, 0);
    assert.equal(refreshes, 1);
...
test("execution model blocks billing, stale consent, and assignment drift before dispatch", async () => {
  ...
  try {
    let calls = 0;
    const billed = connection({
      cost: { ...connection().cost, billingEnabled: true, zeroCost: false },
    });
...
test("source screening permits schema and test references to secret fields but blocks credential values", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "execution-model-source-screening-"),
  );
  try {
    let calls = 0;
    const stored = connection();
    ...
    const base = {
      ...
      permit,
      system: "Return JSON.",
      instruction: "Review bounded changes.",
      responseSchema: { type: "object" },
    };
    ...
    await model.run({
      ...
      sources: [
        {
          name: "tests/integration.test.ts",
          content: 'assert.deepEqual(value, { secret: "must-not-persist" });',
        },
      ],
    });
    assert.equal(calls, 1);
    ...
    await assert.rejects(
      () =>
        model.run({
          ...base,
          taskId: "plan_2222222222222222",
          sources: [
            {
              name: "src/config.ts",
              content: `const apiKey = "${credentialLikeValue}";`,
            },
          ],
        }),
      /sensitive or oversized/i,
    );
    assert.equal(calls, 1);
  } finally {
    await rm(root, { recursive: true, force: true });
