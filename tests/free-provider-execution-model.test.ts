...
test("execution model admits an exact free assignment, records it, and replays durable output", async () => {
  ...
  try {
    ...
    const permit = { schemaVersion: 1 as const, projectId, contextDigest: "a".repeat(64), dataClass: "source_code" as const, providerIds: ["cerebras"], approvedAt: now - 1, expiresAt: now + 60_000 };
    const candidates = await model.candidates(permit, "implementer");
    assert.equal(candidates.length, 1);
    assert.equal(candidates[0]?.billingEnabled, false);
    const input = { projectId, taskId, assignment: { providerId: "cerebras", modelId: "gpt-oss-120b", deviceId: `provider:${stored.id}` }, role: "implementer" as const, permit, system: "Return JSON.", instruction: "Propose bounded changes.", sources: [{ name: "src/feature.ts", content: "export const value = 1;" }], responseSchema: { type: "object" } };
    assert.equal((await model.run(input)).providerId, "cerebras");
    assert.equal((await model.run(input)).providerId, "cerebras");
...
test("execution model blocks billing, stale consent, and assignment drift before dispatch", async () => {
  ...
  try {
    ...
    await assert.rejects(() => model.run({ projectId, taskId, assignment: { providerId: "cerebras", modelId: "wrong", deviceId: `provider:${billed.id}` }, role: "implementer", permit, system: "Return JSON.", instruction: "Work.", sources: [], responseSchema: {} }), /changed before execution/);
    assert.equal(calls, 0);
  } finally { await rm(root, { recursive: true, force: true }); }
});

function connection(overrides: Partial<ProviderConnection> = {}): ProviderConnection { return { schemaVersion: 1, id: "connection-cerebras", providerId: "cerebras", modelId: "gpt-oss-120b", apiBaseUrl: "https://api.cerebras.ai/v1", credentialReference: "vault:providers/cerebras/primary", credentialFingerprint: "012345abcdef", credentialState: "active", state: "ready", privacyClass: "training_eligible", capabilityRoles: ["implementer", "reviewer"], contextWindowTokens: 131_000, maxOutputTokens: 40_000, cost: { access: "account_limited_free", plan: "Free", zeroCost: true, billingEnabled: false, observedAt: now - 1, expiresAt: now + 60_000, source: "account_api" }, quota: { source: "account_api", observedAt: now - 1, expiresAt: now + 60_000, requestsPerMinute: 5, requestsPerDay: 100, tokensPerMinute: 30_000, tokensPerDay: 1_000_000, remainingRequests: 90, remainingTokens: 900_000, resetAt: now + 60_000 }, canary: { status: "passed", observedAt: now - 1, expiresAt: now + 60_000, modelId: "gpt-oss-120b", capabilities: ["chat", "structured_output"], inputTokens: 1, outputTokens: 1, failureCode: null }, updatedAt: now - 1, ...overrides }; }
