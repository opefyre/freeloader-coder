  ...
  reviews: [
    {
      ...
      reviewerId: "delivery-reviewer",
      discipline: "delivery" as const,
      verdict: "pass" as const,
      findings: [],
    },
    {
      schemaVersion: 1 as const,
      reviewerId: "technical-reviewer",
      discipline: "technical" as const,
      verdict: "pass" as const,
      findings: [],
    },
  ],
...
const candidate = {
  ...
  privacyClasses: ["source_code" as const],
  quotaAvailable: true,
  billingEnabled: false,
  activeRequests: 0,
  safeConcurrency: 1,
  availableMemoryMb: 8_000,
  requiredMemoryMb: 4_000,
  deviceLoad: 0.2,
  preference: 10,
};
const eligibility = {
  eligibility: async () => ({
    ...
    eligible: true,
    assessment: {
      classification: "major_feature" as const,
      rationale: ["Multi-stage feature."],
      affectedDomains: ["frontend", "backend"],
      estimatedDeveloperHours: 24,
      requiresArchitectureDecision: true,
      confidence: 1,
    },
    evidence: ["Approved major feature."],
    alternatives: [],
    override: null,
    decidedAt: 1,
  }),
...
test("durable execution enforces one lease, ordered validation, independent quorum, and restart evidence", async () => {
  ...
  try {
    ...
    assert.equal(
      (await service.claim(projectId, "worker-b", [candidate])).task,
      null,
    );
    const lease = first.task!.lease!;
    await assert.rejects(
      () => service.heartbeat(projectId, taskId, lease.leaseId, "worker-b"),
      /current unexpired lease owner/,
    );
    now += 1;
    ...
    await assert.rejects(
      () =>
        service.recordValidation(projectId, taskId, lease.leaseId, "worker-a", {
          tier: "full",
          commandLabel: "full checks",
          passed: true,
          exitCode: 0,
          evidenceDigest: evidence,
        }),
      /requires passing fast/,
    );
    ...
    await service.recordValidation(
      ...
      {
        tier: "full",
        commandLabel: "full verification",
        passed: true,
        exitCode: 0,
        evidenceDigest: evidence,
      },
    );
    const reviewed = await service.recordReviews(
      projectId,
      taskId,
      lease.leaseId,
      "worker-a",
    ...
    const completed = await service.recordIntegration(
      projectId,
      taskId,
      lease.leaseId,
      "worker-a",
    ...
    assert.equal(
      (await restarted.get(projectId))?.tasks[0]?.status,
      "completed",
    );
  } finally {
...
test("execution initialization requires eligibility before reading delivery effects", async () => {
  ...
  await assert.rejects(
    () =>
      new ProjectExecutionService(root, plans, jira, () => 100).initialize(
        projectId,
      ),
    /eligibility authority/i,
  );
...
test("execution initialization preserves an expired eligibility decision while its assessed scope is current", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "project-execution-current-eligibility-"),
  );
  try {
    const plans = {
      readDraft: async () => ({
...
test("failed validation permits only bounded healing and expired outcomes require owner review", async () => {
  ...
  try {
    ...
    const healed = await service.assessHealing(
      projectId,
      taskId,
      lease.leaseId,
      "worker-a",
...
test("owner can resume a pre-implementation environment failure after repair", async () => {
  ...
  try {
    ...
    const interrupted = await service.interrupt(
      ...
      lease.leaseId,
      "worker-a",
      "The new repository has no baseline commit.",
    );
    ...
    await assert.rejects(
      () =>
        service.authorizeEnvironmentRetry(projectId, {
          taskId,
          expectedRevision: interrupted.revision,
          rationale: "Retry this repaired environment once more.",
        }),
      /evidence changed/i,
    );
  } finally {
...
test("owner can resume preserved pre-review evidence after repairing the execution runtime", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "project-execution-runtime-retry-"),
  );
  try {
    const service = makeService(root, () => 100);
    await service.initialize(projectId);
    ...
    const failed = await service.recordValidation(
      projectId,
      taskId,
      lease.leaseId,
      "worker-a",
    ...
    await service.assessHealing(projectId, taskId, lease.leaseId, "worker-a", {
      failureClass: "implementation",
      changedFiles: ["src/app.ts"],
      policy: {
        maxAttempts: 2,
        ...
        requiredReviewRoles: ["functional", "security"],
        minimumGoldenScore: 90,
      },
      goldenScore: 100,
    ...
    const interrupted = await service.interrupt(
      ...
      lease.leaseId,
      "worker-a",
      "Dependency preparation was unavailable.",
    );
    const retried = await service.authorizeEnvironmentRetry(projectId, {
      taskId,
      expectedRevision: interrupted.revision,
      rationale: "Verified dependency preparation is installed.",
    });
    assert.equal(retried.status, "queued");
    assert.equal(
      retried.implementationEvidence.length,
      1,
      "preserved evidence is not erased",
    );
    assert.equal(
      retried.validations.length,
      1,
      "failed evidence remains auditable",
    );
  ...
  try {
    const protectedDraft = {
      ...
      items: draft.items.map((item) =>
        item.id === taskId
          ? { ...item, allowedFiles: [".env.production"] }
          : item,
      ),
    };
    const protectedService = new ProjectExecutionService(
      root,
      {
        readDraft: async () => ({
          ...
          document: {
            schemaVersion: 1,
            projectId,
            projectRelativePath: ".pipeline/BACKLOG.md",
            revision: 1,
            digest,
            markdown: "# Plan",
            itemCount: 4,
    ...
    await assert.rejects(
      () => protectedService.initialize(projectId),
      /protected credential, environment, or Git path/,
    );
    ...
    const claimed = await service.claim(projectId, "worker-a", [candidate]);
    const lease = claimed.task!.lease!;
    const implemented = await service.recordImplementation(
      projectId,
      taskId,
      lease.leaseId,
      "worker-a",
    ...
    assert.equal(
      implemented.status,
      "validating",
      "model implementation evidence alone cannot complete work",
    );
    await service.recordValidation(
      projectId,
      taskId,
...
test("owner-authorized dissent repair preserves rejected evidence and reruns every delivery gate", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "project-execution-review-repair-"),
  );
  try {
    const service = makeService(root, () => 100);
    await service.initialize(projectId);
    ...
    const approval = {
      ...
      rationale:
        "Repair the independently observed design issue without bypassing any gate.",
    };
    const repaired = await service.authorizeReviewRepair(
      projectId,
      taskId,
      approval,
    );
    ...
    assert.equal(
      repaired.reviewAttempts?.[0]?.reviews.some(
    ...
    assert.equal(
      (await service.authorizeReviewRepair(projectId, taskId, approval))
        .revision,
      repaired.revision,
    );

    const restarted = makeService(root, () => 101);
    const secondClaim = await restarted.claim(projectId, "worker-b", [
      candidate,
    ]);
    const secondLease = secondClaim.task!.lease!;
    await restarted.recordImplementation(
      projectId,
      taskId,
      secondLease.leaseId,
      "worker-b",
    ...
    await restarted.recordValidation(
      projectId,
      taskId,
      secondLease.leaseId,
      "worker-b",
    ...
    await restarted.recordReviews(
      projectId,
      taskId,
      secondLease.leaseId,
      "worker-b",
      [
        review("functional-reviewer-2", "gemini", "functional"),
        review("design-reviewer-2", "cloudflare", "design"),
      ],
    );
    const completed = await restarted.recordIntegration(
      projectId,
      taskId,
      secondLease.leaseId,
      "worker-b",
    ...
    assert.equal(
      completed.reviewAttempts?.[0]?.reviews.some(
        (item) => item.verdict === "fail",
      ),
      true,
    );
...
test("owner can repair a contradictory pass review that contains a blocking finding", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "project-execution-blocking-pass-repair-"),
  );
  try {
    const service = makeService(root, () => 100);
    await service.initialize(projectId);
      ...
      findings: [
        {
          ...
          confidence: 0.95,
          acceptanceCriterion: "Large result sets remain navigable.",
          recommendedRepair: "Add accessible pagination after the threshold.",
        },
      ],
    };
    const dissent = await service.recordReviews(
      ...
      lease.leaseId,
      "worker-a",
      [review("functional-reviewer", "gemini", "functional"), contradictory],
    );
    ...
    const repaired = await service.authorizeReviewRepair(projectId, taskId, {
      ...
      expectedRevision: dissent.revision,
      rationale: "Repair the blocking finding and rerun every delivery gate.",
    });
    assert.equal(repaired.status, "queued");
    assert.match(
      repaired.reviewAttempts?.at(-1)?.reviews[1]?.findings[0] ?? "",
      /^major:/,
    );
  } finally {
    await rm(root, { recursive: true, force: true });
...
test("owner-authorized provider proposal repair is bounded and archives the failed attempt", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "project-execution-proposal-repair-"),
  );
  try {
    const service = makeService(root, () => 100);
    await service.initialize(projectId);
    ...
    const interrupted = await service.interrupt(
      ...
      lease.leaseId,
      "worker-a",
      "Execution needs attention: Provider proposal exceeded grounded file authority.",
      "implementation",
    );
    const repaired = await service.authorizeReviewRepair(projectId, taskId, {
      ...
      expectedRevision: interrupted.revision,
      rationale:
        "Retry the bounded provider proposal with exact immutable file authority.",
    });
    assert.equal(repaired.status, "queued");
...
test("owner can repair a legacy assigned failure that stopped before any implementation evidence", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "project-execution-legacy-pre-evidence-repair-"),
  );
  try {
    const service = makeService(root, () => 100);
    await service.initialize(projectId);
    ...
    const stopped = await service.interrupt(
      ...
      lease.leaseId,
      "worker-a",
      "Execution needs attention: strict parser rejected a provider response.",
    );
    ...
    const repaired = await service.authorizeReviewRepair(projectId, taskId, {
      ...
      rationale:
        "Retry the pre-evidence proposal under the corrected strict contract.",
    });
    assert.equal(repaired.status, "queued");
    assert.equal(repaired.attempt, 0);
    assert.equal(
      repaired.reviewAttempts?.at(-1)?.implementationEvidence.length,
      0,
    );
  } finally {
    await rm(root, { recursive: true, force: true });
...
test("owner can repair a provider proposal that omitted a required task file", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "project-execution-omitted-file-repair-"),
  );
  try {
    const service = makeService(root, () => 100);
    await service.initialize(projectId);
    ...
    const interrupted = await service.interrupt(
      ...
      lease.leaseId,
      "worker-a",
      "Execution needs attention: Provider proposal omitted required task files: tests/app.test.ts.",
      "implementation",
    );
    const repaired = await service.authorizeReviewRepair(projectId, taskId, {
      ...
      expectedRevision: interrupted.revision,
      rationale:
        "Retry with every required task-owned source and test file present.",
    });
    assert.equal(repaired.status, "queued");
    assert.equal(repaired.reviewAttempts?.length, 1);
  } finally {
...
test("owner can repair a formatter parse failure before implementation evidence is recorded", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "project-execution-formatter-repair-"),
  );
  try {
    const service = makeService(root, () => 100);
    await service.initialize(projectId);
    ...
    const interrupted = await service.interrupt(
      ...
      lease.leaseId,
      "worker-a",
      "Execution needs attention: Command failed: /isolated/node_modules/.bin/prettier --write src/app.ts\n[error] src/app.ts: SyntaxError: Expression expected.",
      "implementation",
    );
    const repaired = await service.authorizeReviewRepair(projectId, taskId, {
      ...
      expectedRevision: interrupted.revision,
      rationale:
        "Repair the malformed bounded source before rerunning every gate.",
    });
    assert.equal(repaired.status, "queued");
    assert.equal(repaired.attempt, 0);
    assert.equal(repaired.reviewAttempts?.length, 1);
...
test("owner-authorized deterministic validation repair preserves its bounded budget and archives failed proof", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "project-execution-validation-repair-"),
  );
  try {
    const service = makeService(root, () => 100);
    await service.initialize(projectId);
    ...
    const repaired = await service.authorizeReviewRepair(projectId, taskId, {
      ...
      expectedRevision: interrupted.revision,
      rationale:
        "The repaired prerequisite invalidated this deterministic failure; rerun every gate within the remaining bounded budget.",
    });
    assert.equal(repaired.status, "queued");
    assert.equal(
      repaired.attempt,
      0,
      "owner repair starts a new bounded cycle while archived evidence preserves prior attempts",
    );
    assert.equal(
      repaired.reviewAttempts
        ?.at(-1)
        ?.validations.some((validation) => !validation.passed),
      true,
    );
...
test("legacy workspace lifecycle interruption can resume without erasing preserved implementation", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "project-execution-legacy-workspace-"),
  );
  try {
    const service = makeService(root, () => 100);
    await service.initialize(projectId);
    ...
    await service.interrupt(
      ...
      task.lease!.leaseId,
      "worker-a",
      "Execution needs attention: ENOENT: no such file or directory, open '/tmp/project-task-worktrees/task/package.json'",
      "product_decision",
    );
    const interrupted = (await service.get(projectId))!.tasks[0]!;
    const resumed = await service.authorizeEnvironmentRetry(projectId, {
      taskId: interrupted.id,
      expectedRevision: interrupted.revision,
      rationale: "Verified lifecycle fix is active.",
    });
...
test("owner can revalidate unchanged delivered files after a legacy post-review commit-authority stop", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "project-execution-commit-revalidation-"),
  );
  try {
    const service = makeService(root, () => 100);
    await service.initialize(projectId);
    ...
    const repaired = await service.authorizeReviewRepair(projectId, taskId, {
      ...
      expectedRevision: interrupted.revision,
      rationale:
        "Revalidate the unchanged previously delivered files and rerun every deterministic and independent review gate.",
    });
    assert.equal(repaired.status, "queued");
...
test("owner-authorized completed prerequisite repair preserves proof and reopens execution", async () => {
  ...
  try {
    ...
    await service.recordValidation(
      ...
      {
        tier: "fast",
        commandLabel: "initial typecheck",
        passed: false,
        exitCode: 1,
        evidenceDigest: "f".repeat(64),
      },
...
test("owner-facing work cannot start without build and visual journey validation", async () => {
  const root = await mkdtemp(
    join(tmpdir(), "project-execution-ui-acceptance-"),
  );
  try {
    const uiDraft = {
      ...draft,
      items: draft.items.map((item) =>
        item.id === taskId
          ? {
              ...
              description:
                "Implement the approved responsive UI and owner journey.",
...
test("repeated validation failures exhaust the bounded repair budget and quarantine once", async () => {
  ...
  try {
    ...
    for (let cycle = 0; cycle < 3; cycle += 1) {
      task = await service.recordImplementation(
        projectId,
        taskId,
        lease.leaseId,
        "worker-a",
      ...
      task = await service.recordValidation(
        projectId,
        taskId,
        lease.leaseId,
        "worker-a",
        {
          ...
          passed: false,
          exitCode: 1,
          evidenceDigest: `${cycle}`.repeat(64),
        },
      );
      task = await service.assessHealing(
        projectId,
        taskId,
        lease.leaseId,
        "worker-a",
    ...
    await assert.rejects(
      () =>
        service.authorizeQuarantineRecovery(
          projectId,
          {
            taskId,
            ...
            rationale:
              "The deterministic validation root cause was repaired and independently verified.",
          },
          [],
        ),
    ...
    assert.equal(
      recovered.attempt,
      2,
      "recovery preserves the exhausted repair history",
    );
    assert.equal(
      recovered.validations.length,
      3,
      "recovery preserves failed validation evidence",
    );
    assert.match(
      recovered.verifiedRecoveryEvidenceDigest ?? "",
      /^[a-f0-9]{64}$/,
    );
  } finally {
    await rm(root, { recursive: true, force: true });
  ...
  try {
    ...
    const recovered = await service.authorizeQuarantineRecovery(
      projectId,
      {
        ...
        approvalId: "approval_1234567890abcdef1234",
        rationale:
          "The repeated validation root cause was repaired and every required profile was freshly verified.",
      },
      [
        {
          profile: "typecheck",
