...
const implementer = candidate("cerebras", "provider:implementation");
...
test("runtime adapter joins exact provider output, bounded workspace, validation, reviews, integration, and Jira observation", async () => {
  ...
  const model = {
    ...
    run: async (input: any) => {
      ...
      if (input.role === "implementer") return { providerId: "cerebras", modelId: "coder", artifactDigest: "a".repeat(64), response: { summary: "Update feature", operations: [{ type: "replace", path: "src/workflow.ts", content: "export const value = 2;\n", citations: ["src/workflow.ts"], rationale: "Meet the approved behavior." }] } };
  ...
  const permit = { schemaVersion: 1 as const, projectId, contextDigest: "a".repeat(64), dataClass: "source_code" as const, providerIds: ["cerebras", "gemini"], approvedAt: 1, expiresAt: 999_999 };
  ...
  assert.equal((await adapters.candidates(projectId))[0]?.providerId, "cerebras");
...
function candidate(providerId: string, deviceId: string): ExecutionCandidate { return { providerId, modelId: providerId === "cerebras" ? "coder" : "reviewer", deviceId, capabilities: ["chat", "structured_output"], privacyClasses: ["source_code"], quotaAvailable: true, billingEnabled: false, activeRequests: 0, safeConcurrency: 1, availableMemoryMb: 1, requiredMemoryMb: 0, deviceLoad: 0, preference: 10 }; }
function executionTask(): ExecutionTask { return { id: taskId, jiraIssueKey: "PIPE-4", title: "Implement workflow contract", dependsOn: [], allowedFiles: ["src/workflow.ts"], validationProfiles: ["unit"], uiChanged: false, requiredCapabilities: ["chat", "structured_output"], privacyClass: "source_code", status: "running", revision: 1, attempt: 0, assignment: { providerId: "cerebras", modelId: "coder", deviceId: "provider:implementation", selectedAt: 1, reasons: ["All gates passed."] }, lease: { leaseId: "execlease_11111111111111111111", ownerId: "worker", acquiredAt: 1, heartbeatAt: 1, expiresAt: 999_999 }, implementationEvidence: [], validations: [], reviews: [], commitDigest: null, integrationDigest: null, failureClass: null, safeMessage: "Running.", updatedAt: 1 }; }
