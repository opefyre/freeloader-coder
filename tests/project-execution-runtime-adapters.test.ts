...
test("runtime adapter joins exact provider output, bounded workspace, validation, reviews, integration, and Jira observation", async () => {
  ...
  const model = {
    candidates: async (_permit: unknown, role: string) =>
      role === "implementer" ? [implementer] : [reviewer, secondReviewer],
    run: async (input: any) => {
      ...
      if (input.role === "implementer") {
        ...
        assert.equal(
          input.sources.some(
            (source: { name: string }) =>
              source.name === `delivery-plan://${taskId}`,
          ),
          true,
        );
        assert.deepEqual(
          input.responseSchema.properties.operations.items.properties.path.enum,
          ["src/workflow.ts"],
        );
        assert.deepEqual(
        ...
        assert.match(
          instruction.toolchainCompatibilityRule,
          /when it runs node --test, import test from node:test and import assert from node:assert\/strict/,
        );
        assert.match(
          instruction.toolchainCompatibilityRule,
          /node:test has no named strict assertion export/,
        );
        assert.match(
          instruction.toolchainCompatibilityRule,
          /react-dom\/client does not export createElement/,
        );
        assert.match(
          instruction.toolchainCompatibilityRule,
        ...
        assert.match(
          instruction.toolchainCompatibilityRule,
          /Never import a test framework that the configured command does not launch/,
        );
        assert.match(
          instruction.toolchainCompatibilityRule,
          /NodeNext and Node16 require the emitted \.js suffix/,
        );
        assert.match(
          instruction.toolchainCompatibilityRule,
          /when browser globals are not declared, use globalThis\.document/,
        );
        assert.match(
          instruction.toolchainCompatibilityRule,
          /A \.ts file cannot contain JSX syntax/,
        );
        assert.match(
          instruction.authorityRule,
          /Never import a project-local module unless that module is present in the supplied sources/,
        );
        assert.match(
          instruction.authorityRule,
          /declare and export it inside an allowed implementation file/,
        );
        return {
          providerId: "groq",
          modelId: "coder",
            ...
            operations: [
              {
                ...
                citations: ["src/workflow.ts"],
                rationale: "Meet the approved behavior.",
              },
            ],
          },
        };
      }
      reviewOutputBudgets.push(input.maxOutputTokens);
      const instruction = JSON.parse(input.instruction);
      assert.deepEqual(
        instruction.currentValidationEvidence.map(
          (item: { tier: string; passed: boolean }) => ({
            tier: item.tier,
            passed: item.passed,
          }),
        ),
        [
          { tier: "fast", passed: true },
          ...
          findings: [
            {
              ...
              evidenceRef: digest,
              confidence: 0.99,
              acceptanceCriterion: "Approved behavior is present.",
              recommendedRepair:
                role === "functional" ? "" : "No repair is required.",
            },
          ],
        },
      };
  ...
  const workspaces = {
    ...
    sources: async (_workspace: unknown, sourceTask: ExecutionTask) => {
      if (sourceTask.allowedFiles.includes("package.json"))
        sawToolchainContext = true;
      return [
        {
          path: "src/workflow.ts",
    ...
    apply: async (_workspace: unknown, _task: unknown, operations: any[]) => {
      calls.push("apply");
      assert.equal(operations[0].expectedBeforeDigest, "c".repeat(64));
      return {
    ...
    commit: async () => ({
      commitDigest: "f".repeat(40),
      evidenceDigest: digest,
    }),
    integrate: async () => ({ integrationDigest: digest }),
    revertIntegration: async () => {
      throw new Error("must not revert passing integration");
    },
  };
  const permit = {
  ...
  assert.equal(
    (await adapters.candidates(projectId, task))[0]?.providerId,
    "groq",
  );
  assert.equal((await adapters.healingPolicy(projectId, task)).maxAttempts, 5);
  assert.equal(
    (await adapters.healingPolicy(projectId, { ...task, attempt: 7 }))
      .maxAttempts,
    5,
  );
  assert.equal(
    (await adapters.healingPolicy(projectId, { ...task, attempt: 9 }))
      .maxAttempts,
    5,
  );
  ...
  const reviews = await adapters.review(projectId, {
    ...task,
    validations: [
      {
        tier: "fast",
  ...
  assert.equal(
    reviews[0]?.findings[0]?.recommendedRepair,
    "No repair required.",
  );
...
test("runtime adapter grounds a first source file in the reviewed delivery task", async () => {
  ...
  const model = {
    ...
    run: async (input: any) => {
      ...
      return {
        ...
        artifactDigest: "a".repeat(64),
        response: {
          summary: "Create first feature",
          operations: [
            {
              type: "replace",
              path: "src/workflow.ts",
              content: "export const value = 1;\n",
              citations: [`delivery-plan://${taskId}`],
              rationale: "Implement the reviewed task.",
            },
          ],
        },
      };
  ...
  const workspaces = {
    ...
    apply: async (_workspace: unknown, _task: unknown, operations: any[]) => {
      ...
      assert.equal(operations[0].expectedBeforeDigest, null);
      return {
        changedFiles: ["src/workflow.ts"],
        evidenceDigest: "e".repeat(64),
      };
  ...
  const adapters = new ProjectExecutionRuntimeAdapters(
    ...
    {
      readDraft: async () => ({
        draft: { ...completeDeliveryPlan(), revision: 1, reviews: [] } as any,
      }),
    },
    { readVerified: async () => ({ digest: permit.contextDigest }) },
    { authorize: async () => permit },
...
test("runtime adapter rejects JSX in a .ts proposal before workspace mutation", async () => {
          ...
          operations: [
            {
              ...
              citations: ["src/workflow.ts"],
              rationale: "Render the owner view.",
            },
          ],
        },
      }),
  ...
  await assert.rejects(
    () => adapters.implement(projectId, executionTask(), 0),
    /failed strict response contract.*invalid TypeScript syntax/,
  );
...
test("a rejected implementation cycle rotates to another eligible free provider", async () => {
  ...
  const candidates = await adapters.candidates(projectId, {
    ...executionTask(),
    reviewAttempts: [rejectedAttempt],
  });
  assert.deepEqual(
...
test("scaffold implementation requires project-wide test discovery", async () => {
  ...
  const scaffoldTask = {
    ...executionTask(),
    allowedFiles: ["package.json", "tests/scaffold.test.ts"],
  };
  const adapters = new ProjectExecutionRuntimeAdapters(
    { canonicalRoot: async () => "/canonical" },
    {
      readDraft: async () => ({
        draft: {
          ...
          items: [
            {
              ...completeDeliveryPlan().items[0]!,
              id: taskId,
              allowedFiles: scaffoldTask.allowedFiles,
            },
          ],
        } as any,
      }),
    ...
    { authorize: async () => permit },
    {
      ...
      run: async (input: any) => {
        ...
        return {
          ...
          artifactDigest: digest,
          response: {
            summary: "Create complete scaffold",
            operations: [
              {
                type: "create",
                path: "package.json",
                content: "{}\n",
                citations: [`delivery-plan://${taskId}`],
                rationale: "Create manifest.",
              },
              {
                type: "create",
                path: "tests/scaffold.test.ts",
                content: "export {};\n",
                citations: [`delivery-plan://${taskId}`],
    ...
    },
    {
      ...
      sources: async () => [],
      apply: async () => ({
        changedFiles: scaffoldTask.allowedFiles,
        evidenceDigest: digest,
      }),
    } as any,
    { synchronize: async () => undefined },
  ...
  assert.match(
    instruction,
    /Every package imported by lint, compiler, test, or build configuration must be declared/,
  );
  assert.match(
    instruction,
    /configuration APIs must match the declared package versions/,
  );
  assert.match(
    instruction,
    /\.pipeline\/, \.codkesh\/, and governed root Markdown documents/,
  );
});
  ...
  const adapters = new ProjectExecutionRuntimeAdapters(
    ...
    { authorize: async () => permit },
    {
      ...
      run: async () => ({
        ...
        artifactDigest: "a".repeat(64),
        response: {
          summary: "Repair observed file",
          operations: [
            {
              type: "create",
              path: "src/workflow.ts",
              content: "export const value = 2;\n",
              citations: [`delivery-plan://${taskId}`, "src/workflow.ts"],
              rationale: "Repair the observed implementation.",
            },
          ],
        },
      }),
    },
    {
      ...
      apply: async (_workspace: unknown, _task: unknown, operations: any[]) => {
        observedType = operations[0].type;
        assert.equal(operations[0].expectedBeforeDigest, digest);
        return { changedFiles: ["src/workflow.ts"], evidenceDigest: digest };
      },
    } as any,
    { synchronize: async () => undefined },
  );
  ...
  const adapters = new ProjectExecutionRuntimeAdapters(
    ...
    { authorize: async () => permit },
    {
      ...
      run: async () => ({
        ...
        artifactDigest: "a".repeat(64),
        response: {
          summary: "Preserve and refine implementation",
          operations: [
            {
              type: "replace",
              path: "src/workflow.ts",
              content: "export const value = 2;\n",
              citations: ["src/workflow.ts"],
              rationale: "Apply the bounded validation correction.",
            },
          ],
        },
      }),
    ...
    reviewAttempts: [
      {
        ...
        reviews: [],
        rationale: "Retry the deterministic validation correction.",
        decidedAt: 2,
      },
    ],
  };
...
test("independent review rotates to another eligible free provider when one rejects the request", async () => {
  ...
  const secondCompatible = candidate(
    "gemini",
  ...
  const permit = {
    ...
    contextDigest: "a".repeat(64),
    dataClass: "source_code" as const,
    providerIds: ["groq", "kilo", "mistral", "gemini"],
    approvedAt: 1,
    expiresAt: 999_999,
  };
  const adapters = new ProjectExecutionRuntimeAdapters(
    ...
    { authorize: async () => permit },
    {
      candidates: async (_permit: unknown, role: string) =>
        role === "implementer"
          ? [implementer]
          : [rejected, compatible, secondCompatible],
      run: async (input: any) => {
        if (input.role === "implementer")
          return {
            providerId: "groq",
            ...
            artifactDigest: "a".repeat(64),
            response: {
              summary: "Prepare review fixture",
              operations: [
                {
                  type: "replace",
                  path: "src/workflow.ts",
                  content: "export const value = 2;\n",
                  citations: ["src/workflow.ts"],
                  rationale: "Prepare the reviewed change.",
                },
              ],
            },
          };
        ...
        if (input.assignment.providerId === "kilo")
          return {
            ...
            artifactDigest: "b".repeat(64),
            response: {
              reviewerId: "contradictory-reviewer",
              verdict: "pass",
              findings: [
                {
                  id: "blocking",
                  ...
                  confidence: 0.9,
                  acceptanceCriterion: "Approved behavior is present.",
                  recommendedRepair: "Change the approved behavior.",
                },
              ],
            },
          };
    ...
    },
    {
      ...
      sources: async () => [
        { path: "src/workflow.ts", content: "export const value = 2;", digest },
      ],
      apply: async () => ({
        changedFiles: ["src/workflow.ts"],
        evidenceDigest: digest,
  ...
  assert.ok(
    reviewSystems.every((system) =>
      system.includes(
        "must not fail code for an alleged parse, format, lint, typecheck, build, or test error",
      ),
    ),
  );
  assert.ok(
    reviewSystems.every((system) =>
      system.includes("tsconfig.json is TypeScript JSONC"),
    ),
  );
});
...
test("temporary review-quorum shortage remains retryable instead of becoming owner work", async () => {
  ...
  const adapters = new ProjectExecutionRuntimeAdapters(
    ...
    },
    {
      ...
      sources: async () => [
        { path: "src/workflow.ts", content: "export const value = 1;", digest },
      ],
      apply: async () => ({
        changedFiles: ["src/workflow.ts"],
        evidenceDigest: digest,
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      ...
      /review quorum/i.test(error.message),
  );
});

test("review reopens its deterministic task workspace after a controller restart", async () => {
  let prepared = 0;
  const workspace = {
    projectId,
    taskId,
  ...
  const adapters = new ProjectExecutionRuntimeAdapters(
    ...
    { authorize: async () => permit },
    {
      candidates: async () => [reviewer, secondReviewer],
      run: async (input: any) => ({
        providerId: input.assignment.providerId,
        modelId: "reviewer",
        artifactDigest: digest,
        response: {
          reviewerId: "restart-reviewer",
          verdict: "pass",
          findings: [],
        },
      }),
...
function candidate(providerId: string, deviceId: string): ExecutionCandidate {
  return {
    providerId,
    modelId: providerId === "groq" ? "coder" : "reviewer",
    deviceId,
    capabilities: ["chat", "structured_output"],
    privacyClasses: ["source_code"],
    quotaAvailable: true,
...
function executionTask(): ExecutionTask {
  return {
    ...
    dependsOn: [],
    allowedFiles: ["src/workflow.ts"],
    validationProfiles: ["unit"],
    uiChanged: false,
    ...
    assignment: {
      providerId: "groq",
      modelId: "coder",
      deviceId: "provider:implementation",
      selectedAt: 1,
      reasons: ["All gates passed."],
    },
    lease: {
      leaseId: "execlease_11111111111111111111",
      ownerId: "worker",
      acquiredAt: 1,
      heartbeatAt: 1,
      expiresAt: 999_999,
