...
test("relay authenticates Slack, stores only opaque decision metadata, and permits one authorized pull", async () => {
  ...
  const payload = { type: "block_actions", user: { id: "U-owner", name: "personal-name" }, channel: { id: "C-owner", name: "private-name" }, actions: [{ action_id: "codkesh_owner_response", value: "decision_0123456789abcdef" }], message: { text: "private project source" } };
