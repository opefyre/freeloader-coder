...
test("OAuth owner identity resolver binds stable and legacy connection aliases to the authenticated human", async () => {
  const secrets = new Map([
    ["vault:providers/slack/default", JSON.stringify({ accessToken: "slack-access-token" })],
  ...
  const resolver = new OAuthOwnerIdentityResolver(
    ...
    async (input) => {
      ...
      if (url.includes("slack.com")) return new Response(JSON.stringify({ ok: true, user_id: "U-owner", team_id: "T-stable", team: "Workspace" }), { status: 200 });
