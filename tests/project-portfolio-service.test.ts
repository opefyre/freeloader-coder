    ...
    projects: [
      {
        ...
        resources: [
          {
            id: "binding_0123456789abcdef",
            kind: "jira_project",
            connectionId: "jira:default",
            resourceId: "10132",
            label: "PIPE · Coding Pipeline",
            url: "https://opefyre.atlassian.net/jira/software/projects/PIPE",
            role: "primary",
...
test("portfolio derives progress from selected Jira and latest state from canonical execution", async () => {
  ...
  const service = new ProjectPortfolioService(
      ...
      list: async () => [
        {
          ...
          stage: "delivery",
          revision: 4,
          mission: "Deliver the project portfolio",
          questions: [],
          answers: [],
          assessment: null,
          artifacts: [],
    ...
    },
    {
      read: async () =>
        JSON.stringify({
          siteUrl: "https://opefyre.atlassian.net",
        ...
        issues: [
          {
            key: "PIPE-99",
            fields: {
              summary: "Implement portfolio",
              updated: "2026-08-09T11:59:00.000Z",
              status: {
                name: "In Progress",
...
test("portfolio labels Jira as unknown when authentication or observation is unavailable", async () => {
  const service = new ProjectPortfolioService(
    ...
    { read: async () => null },
    async () => {
      throw new Error("must not request Jira without credentials");
    },
    () => now,
  );
...
test("portfolio observes the selected Jira project through the browser OAuth credential", async () => {
  ...
  const service = new ProjectPortfolioService(
    ...
    async (input, init) => {
      ...
      if (String(input).includes("accessible-resources"))
        return Response.json([
          { id: "cloud-123", url: "https://opefyre.atlassian.net" },
        ]);
      return Response.json({
        isLast: true,
        issues: [
  ...
  assert.ok(
    urls.some(
      (url) =>
        url ===
        "https://api.atlassian.com/ex/jira/cloud-123/rest/api/3/search/jql?jql=project+%3D+%22PIPE%22+ORDER+BY+updated+DESC&maxResults=100&fields=summary%2Cstatus%2Cupdated",
    ),
  );
});

test("portfolio does not reopen a fully reconciled completed project from historical execution", async () => {
  const service = new ProjectPortfolioService(
    { list: async () => collection() },
    {
      list: async () => [
        {
          ...
          stage: "complete",
          revision: 9,
          mission: "Complete the owner journey",
          questions: [],
          answers: [],
          assessment: null,
          artifacts: [],
        ...
        tasks: [
          {
            ...
            safeMessage: "Historical environment failure.",
            updatedAt: now - 5_000,
          },
        ],
      }),
  ...
  const project = (await service.list()).projects[0]!;
  assert.equal(project.lifecycleStage, "complete");
  assert.equal(project.state, "ready");
  assert.equal(project.progress?.percent, 100);
  assert.equal(project.reconciliation?.confidence, "verified");
  assert.deepEqual(project.reconciliation?.disagreements, []);
  assert.equal(project.latestUpdate?.source, "jira");
});

test("shared Jira bindings retain each local project's identity in the freshness cache", async () => {
  const secondProjectId = "project_fedcba9876543210";
  ...
  assert.deepEqual(
    observed.projects.map((project) => project.reconciliation?.projectId),
    [projectId, secondProjectId],
  );
  assert.ok(
    observed.projects.every(
      (project) => project.reconciliation?.confidence === "verified",
    ),
  );
  assert.ok(
    observed.projects.every(
      (project) => project.reconciliation?.disagreements.length === 0,
    ),
  );
});
