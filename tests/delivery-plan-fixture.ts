export function completeDeliveryPlan() {
  ...
  const common = { priority: "high" as const, acceptanceCriteria: ["The intended behavior is verified with deterministic evidence.", "Failure behavior is covered without losing prior work."], definitionOfDone: ["Automated validation passes in the supported environment.", "Independent review finds no unresolved blocking issue."], implementationNotes: ["Follow the approved architecture and existing repository conventions."], roleCapabilities: ["typescript implementation", "independent validation"], rollbackRequirements: ["Restore the last verified revision if post-change validation fails."], allowedFiles: ["src/workflow.ts", "tests/workflow.test.ts"], validationProfiles: ["typecheck" as const, "unit" as const], citations: ["local://CONTEXT.md", "local://.pipeline/SOLUTION.md"] };
  return {
    ...
    coverage: (["behavior", "architecture", "user_experience", "data", "integrations", "security", "privacy", "reliability", "rollout", "metrics"] as const).map((requirement) => ({ requirement, itemIds: [subtask], validationProfiles: ["unit" as const], citations: common.citations })),
    gates: [{ id: "gate_0000000000000001", kind: "owner_approval" as const, title: "Approved solution boundary", rationale: "Implementation starts only after the owner approves the exact researched solution digest.", beforeItemIds: [subtask] }],
  };
}
