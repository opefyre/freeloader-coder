...
test("owner channel runtime delivers once across restarts and reconciles responses", async () => {
  ...
  assert.deepEqual(await first.synchronize(), { planned: 1, delivered: 1, deferred: 0, applied: 1, pending: 0 });
  ...
  assert.deepEqual(await restarted.synchronize(), { planned: 1, delivered: 0, deferred: 0, applied: 1, pending: 0 });
...
test("failed and unavailable transports remain retryable without blocking another provider", async () => {
  ...
  assert.deepEqual(await runtime.synchronize(), { planned: 2, delivered: 0, deferred: 2, applied: 0, pending: 0 });
  assert.deepEqual(await runtime.synchronize(), { planned: 2, delivered: 0, deferred: 2, applied: 0, pending: 0 });
...
test("relay failure is contained and corrupt runtime state fails closed", async () => {
  ...
  assert.deepEqual(await runtime.synchronize(), { planned: 0, delivered: 0, deferred: 0, applied: 0, pending: 1 });
