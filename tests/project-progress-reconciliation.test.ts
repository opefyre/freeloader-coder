  ...
  const result = reconcileProjectProgress({
    ...
    jira: {
      ...
      latest: {
        ...
        summary: "Jira update",
        occurredAt: now - 50,
        url: "https://example.atlassian.net/browse/PIPE-2",
      },
      issues: new Map([
        ["PIPE-1", "done"],
        ["PIPE-2", "active"],
...
test("missing Jira never invents progress and stale or conflicting evidence is explicit", () => {
  const missing = reconcileProjectProgress({
    projectId,
    jira: null,
    execution: { projectId, observedAt: now, latest: null, issues: new Map() },
    lifecycle: null,
    now,
  });
  ...
  const conflict = reconcileProjectProgress({
    ...
    jira: {
      ...
      freshUntil: now - 1,
      latest: null,
      issues: new Map([["PIPE-1", "todo"]]),
    },
    execution: {
      projectId,
      observedAt: now,
      latest: null,
      issues: new Map([["PIPE-1", "done"]]),
    },
    lifecycle: null,
    now,
  });
  ...
  assert.deepEqual(
    conflict.disagreements.map((item) => item.code),
    ["task_state_conflict", "stale_source"],
  );
});
...
test("cross-project evidence is rejected and cannot contaminate metrics", () => {
  ...
  assert.deepEqual(
    result.disagreements.map((item) => item.code),
    ["project_mismatch", "missing_jira"],
  );
});

test("verified terminal Jira completion supersedes stale historical execution state", () => {
  const result = reconcileProjectProgress({
    projectId,
    jira: {
      projectId,
    ...
    execution: {
      ...
      latest: {
        issueKey: "PIPE-2",
        summary: "Historical environment failure",
    ...
    lifecycle: { stage: "complete", updatedAt: now - 100 },
    now,
  });
  assert.equal(result.confidence, "verified");
  assert.deepEqual(result.disagreements, []);
  assert.equal(result.latest?.source, "jira");
