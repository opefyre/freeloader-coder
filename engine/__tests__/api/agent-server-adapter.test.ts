...
describe("agent_settings runtime services suffix", () => {
  it("adds workspace grounding without replacing an agent profile suffix", () => {
    const contextual = attachWorkspaceContext(
      {
        agent_context: { system_message_suffix: "Owner-approved profile rule." },
      },
      "/Users/example/product",
    );
    const suffix = (contextual.agent_context as Record<string, unknown>)
      .system_message_suffix as string;

    expect(suffix).toContain("Owner-approved profile rule.");
    expect(suffix).toContain("<PIPELINE_WORKSPACE_CONTEXT>");
    expect(suffix).toContain("Workspace: /Users/example/product");
  });
  ...
  it("keeps pipeline/auto requests lean while preserving project grounding", () => {
    const payload = buildStartConversationRequest({
      settings: {
        ...DEFAULT_SETTINGS,
        agent_settings: {
          ...(DEFAULT_SETTINGS.agent_settings ?? {}),
          llm: {
            ...(((DEFAULT_SETTINGS.agent_settings ?? {}) as Record<string, unknown>).llm as Record<string, unknown>),
            model: "openai/pipeline/auto",
          },
        },
      },
      query: "hello",
  ...
  it("preserves project grounding when runtime services are appended", () => {
    const payload = buildStartConversationRequest({
      settings: {
        ...DEFAULT_SETTINGS,
        agent_settings: {
          ...DEFAULT_SETTINGS.agent_settings,
          agent_context: {
            system_message_suffix: "<PIPELINE_PROJECT_CONTEXT>grounded</PIPELINE_PROJECT_CONTEXT>",
          },
        },
      },
      query: "hello",
    ...
    const suffix = payload.agent_settings.agent_context
      .system_message_suffix as string;
    expect(suffix).toContain("<PIPELINE_PROJECT_CONTEXT>grounded");
    expect(suffix).toContain("<RUNTIME_SERVICES>");
  });
});
