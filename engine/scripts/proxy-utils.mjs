...
export function createProxyHandlers({
  ...
  const proxy = createProxyServer({
    ...
    changeOrigin: true,
    xfwd: true,
    // The local ingress handles long-lived UI polling. Reusing one upstream
    // socket caused the proxy dependency to accumulate timeout listeners on
    // that socket, eventually emitting MaxListenersExceededWarning. A local
    // non-pooled connection keeps each request lifecycle isolated.
    agent: false,
    timeout,
    proxyTimeout,
  });
