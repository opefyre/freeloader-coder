...
// The SDK currently targets FastMCP 3 / MCP 1. An unconstrained `fastmcp>=3`
// can resolve the FastMCP 4 beta and MCP 2, whose low-level Server API no
// longer exposes `list_tools`; conversation creation then fails before the
// first model call. Keep the launcher on the compatible protocol generation.
const FAST_MCP_CONSTRAINT = SHARED_DEFAULTS.constraints?.fastMcp;
...
export function buildAgentServerCommand(env = process.env) {
  ...
  if (localPath) {
    ...
    uvxArgs.push("--with", FAST_MCP_CONSTRAINT, "--with", MCP_CONSTRAINT);
