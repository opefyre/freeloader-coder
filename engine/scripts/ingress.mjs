 ...
 *   INGRESS_HOST          - Address to bind (default: 127.0.0.1)
...
function buildConfig(args, env = process.env) {
  ...
  return {
    ...
    host: args.host || env.INGRESS_HOST || "127.0.0.1",
    routes,
...
export function startIngress(config) {
  ...
  server.listen(config.port, config.host, () => {
    ...
    console.log(
      `║  Listening on: http://${config.host}:${config.port}/`.padEnd(66) + "║",
    );
    console.log(
      "╠═══════════════════════════════════════════════════════════════╣",
    );
