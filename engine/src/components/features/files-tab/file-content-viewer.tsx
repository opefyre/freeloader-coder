...
export function FileContentViewer({ path, viewMode }: FileContentViewerProps) {
  ...
  if (mimeType === "text/html" || HTML_LIKE_EXTS.has(getExtension(path))) {
    // Rich HTML is interactive so script-based styling systems such as the
    // Tailwind CDN can initialize. We deliberately omit `allow-same-origin`:
    // the preview receives an opaque origin and cannot reach Pipeline
    // Studio's DOM, storage, cookies, or authenticated APIs.
    return (
      <iframe
        title={path}
        src={bustedStaticUrl}
