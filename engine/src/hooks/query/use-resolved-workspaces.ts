 ...
 */
// Desktop/local users normally choose real folders from their machine. The
// conventional `/projects` root exists in some container deployments only;
// probing it on every development page load creates a guaranteed 404 on macOS.
// Keep the convenience available as an explicit deployment opt-in.
const INCLUDE_IMPLICIT_WORKSPACE_PARENTS =
  import.meta.env.VITE_ENABLE_IMPLICIT_PROJECTS_ROOT === "true";
