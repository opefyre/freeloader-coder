...
import { buildWorkspaceContextSuffix } from "#/pipeline/project-context";
import {
...
function buildAgentContext(
  ...
  const existingSystemSuffix =
    typeof existingContext.system_message_suffix === "string"
      ...
      : "";
  const combinedSystemSuffix = [existingSystemSuffix, runtimeServicesSuffix]
  ...
  const existingSkills = Array.isArray(existingContext.skills)
    ...
    : [];
  const pipelineAuto =
    toRecord(agentSettings.llm).model === "openai/pipeline/auto";
  const disabledSkillNames = new Set(disabledSkills);
  const mergedSkills = [
    ...existingSkills,
    ...(pipelineAuto ? [] : buildBundledSkills()),
  ].filter(
    (skill) =>
  ...
  return {
    ...
    // The free-tier router has strict per-request token ceilings. Pipeline
    // auto keeps repository grounding but does not inject the entire global
    // user-skill library into every model call. Skills remain available in
    // the UI and can be invoked explicitly when needed.
    load_user_skills: !pipelineAuto,
    load_project_skills: true,
    ...(combinedSystemSuffix
      ? { system_message_suffix: combinedSystemSuffix }
      : {}),
  };
}
...
export async function buildStartConversationRequestWithEncryptedSettings(options: {
  ...
  const { agentSettings, conversationSettings, secretsEncrypted } =
    settingsResult;
  const contextualAgentSettings = options.workingDir
    ? attachWorkspaceContext(agentSettings, options.workingDir)
    : agentSettings;
  ...
  if (!options.agentProfileId) {
    await assertSubscriptionAuthReady(contextualAgentSettings);
  }

  return buildStartConversationRequest({
    ...options,
    encryptedAgentSettings: contextualAgentSettings,
...
export function attachWorkspaceContext(
  ...
  const agentContext = toRecord(agentSettings.agent_context);
  const existingSuffix =
    typeof agentContext.system_message_suffix === "string"
      ? agentContext.system_message_suffix.trim()
      : "";
  const workspaceSuffix = buildWorkspaceContextSuffix(workingDir);
  return {
    ...agentSettings,
    agent_context: {
      ...agentContext,
      system_message_suffix: [existingSuffix, workspaceSuffix]
        .filter(Boolean)
        .join("\n\n"),
