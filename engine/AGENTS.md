...
- Current `overrides` in `package.json` and the advisories they address (keep this list in sync when adding/removing overrides):
  ...
  - `dompurify: 3.4.14` — keeps Monaco and PostHog above known DOMPurify XSS bypass ranges, including **GHSA-55q2-fjhq-7xh7**.
  - `nanoid: 3.3.18` — keeps PostCSS's generator above **GHSA-2v37-7h3g-55p8** without a breaking major upgrade.
