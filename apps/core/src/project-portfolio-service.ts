...
type JiraCredential = { kind: "basic"; siteUrl: string; email: string; apiToken: string } | { kind: "oauth"; accessToken: string };
type JiraObservation = Pick<LocalProjectSnapshot, "latestUpdate" | "progress"> & { evidence: JiraProgressObservation };

export class ProjectPortfolioService {
  readonly #cache = new Map<string, { expiresAt: number; observation: JiraObservation }>();
  #oauthSites: { expiresAt: number; sites: readonly { id: string; url: string }[] } | null = null;
  ...
  async list(): Promise<LocalProjectCollection> {
    ...
    const projects = await Promise.all(collection.projects.map(async (project) => {
      ...
      const executionEvidence = execution ? executionObservation(execution) : null;
      const reconciliation = reconcileProjectProgress({
        projectId: project.id,
        jira: jira?.evidence ?? null,
        execution: executionEvidence,
        lifecycle: lifecycle ? { stage: lifecycle.stage, updatedAt: lifecycle.updatedAt } : null,
        now: this.now(),
      });
      ...
      return {
        ...
        latestUpdate,
        progress: reconciliation.progress ? { source: "jira" as const, ...reconciliation.progress, percent: reconciliation.progress.percent ?? 0, observedAt: jira?.evidence.observedAt ?? this.now() } : null,
        reconciliation,
        state,
        observedAt: this.now(),
      };
  ...
  async #observeJira(project: LocalProjectSnapshot, credential: JiraCredential): Promise<JiraObservation | null> {
    ...
    const cacheKey = `${selected.origin}:${key}`;
    ...
    const headers = { Accept: "application/json", Authorization: access.authorization };
    ...
    const observation: JiraObservation = {
      ...
      evidence: {
        ...
        latest: latest ? { issueKey: latest.key, summary: latest.summary, occurredAt: latest.updatedAt, url: `${selected.origin}/browse/${encodeURIComponent(latest.key)}` } : null,
  ...
  async #jiraAccess(credential: JiraCredential, selectedOrigin: string) {
    if (credential.kind === "basic") {
      ...
      return { apiBase: credential.siteUrl, authorization: `Basic ${Buffer.from(`${credential.email}:${credential.apiToken}`, "utf8").toString("base64")}` };
    }
    if (!this.#oauthSites || this.#oauthSites.expiresAt <= this.now()) {
      const response = await this.fetcher("https://api.atlassian.com/oauth/token/accessible-resources", { headers: { Accept: "application/json", Authorization: `Bearer ${credential.accessToken}` }, redirect: "error" });
      const text = await response.text();
      if (!response.ok || text.length > 1_000_000) throw new Error("Jira OAuth site discovery failed safely.");
      const body = JSON.parse(text) as unknown;
      if (!Array.isArray(body)) throw new Error("Jira OAuth site discovery returned invalid data.");
      const sites = body.flatMap((item) => item && typeof item === "object" && typeof (item as Record<string, unknown>).id === "string" && typeof (item as Record<string, unknown>).url === "string" ? [{ id: String((item as Record<string, unknown>).id), url: new URL(String((item as Record<string, unknown>).url)).origin }] : []);
      this.#oauthSites = { expiresAt: this.now() + 300_000, sites };
    }
    const site = this.#oauthSites.sites.find((item) => item.url === selectedOrigin);
    return site ? { apiBase: `https://api.atlassian.com/ex/jira/${encodeURIComponent(site.id)}`, authorization: `Bearer ${credential.accessToken}` } : null;
  }
}
...
function parseCredential(value: string): JiraCredential {
  ...
  return { kind: "basic", siteUrl: site.origin, email: parsed.email, apiToken: parsed.apiToken };
}
...
function parseIssue(value: unknown) {
  ...
  return { key: issue.key, summary: issue.fields.summary.slice(0, 450), updatedAt, done: category === "done", blocked, state: blocked ? "blocked" as const : category === "done" ? "done" as const : category === "new" ? "todo" as const : "active" as const };
}

function executionObservation(record: ProjectExecutionRecord): ExecutionProgressObservation {
  const latest = [...record.tasks].sort((left, right) => right.updatedAt - left.updatedAt)[0] ?? null;
  return {
    projectId: record.projectId,
    observedAt: record.updatedAt,
    latest: latest ? { issueKey: latest.jiraIssueKey, summary: latest.safeMessage, occurredAt: latest.updatedAt } : null,
    issues: new Map(record.tasks.map((task) => [task.jiraIssueKey, executionState(task.status)])),
  };
}

function executionState(status: ProjectExecutionRecord["tasks"][number]["status"]): "todo" | "active" | "done" | "blocked" {
