...
export function buildDecisionSnapshot(input: {
  ...
  const canonical = deduplicate([
    ...input.live.recentEvents.flatMap((event) =>
      fromLiveEvent(event, now, input.lifecycles ?? []),
    ),
    ...input.live.providers.flatMap((provider) => fromProvider(provider, now)),
    ...input.autonomy.recommendations.flatMap((recommendation) =>
      supersededByLifecycle(recommendation, input.lifecycles ?? [])
        ? []
        : fromRecommendation(recommendation, now),
    ),
    ...(input.lifecycles ?? []).flatMap((lifecycle) =>
      fromLifecycle(lifecycle, now),
    ),
    ...(input.executions ?? []).flatMap((execution) =>
      terminalLifecycle(execution.projectId, input.lifecycles ?? [])
        ? []
        : fromExecution(execution, now),
    ),
    ...input.autonomy.leases
        ...
        decision(
          {
            ...
            reason:
              "The bounded safe-step lease expired. Canonical reconciliation is required before another worker can proceed.",
        ...
        decision(
          {
            ...
            category: receipt.action.includes("reconcile")
              ? "recovery"
              : "failure",
    ...
    .slice(0, 250);
  const ranged = canonical.filter((item) =>
    inRange(item.observedAt, query.range, now),
  );
  const facets = {
    ...
    projects: facet(
      ranged.filter((item) => item.projectId),
      (item) => item.projectId ?? "",
    ),
    providers: facet(
      ranged.filter((item) => item.providerId),
      (item) => item.providerId ?? "",
    ),
  };
  const needle = normalize(query.search);
  const items = ranged.filter(
    (item) =>
      ...
      (!needle ||
        normalize(
          `${item.title} ${item.reason} ${item.nextAction} ${item.category} ${item.providerId ?? ""}`,
        ).includes(needle)),
  );
  return decisionSnapshotSchema.parse({
    ...
    summary: {
      ...
      blockedProjects: new Set(
        items.filter((item) => item.projectId).map((item) => item.projectId),
      ).size,
      providerWaits: items.filter((item) => item.category === "provider")
        .length,
    ...
    retention: {
      ...
      earliestObservedAt: canonical.length
        ? Math.min(...canonical.map((item) => item.observedAt))
        : null,
    },
    items,
  });
...
function fromExecution(
  ...
  return record.tasks
    .filter(
      (task) => task.status === "needs_user" || task.status === "quarantined",
    )
    .map((task) => {
      ...
      return decision(
        {
          ...
          evidence: [
            ...
            `Execution revision ${task.revision}`,
            `${task.implementationEvidence.length} implementation evidence record(s)`,
            `${task.validations.length} validation record(s)`,
          ],
          reference: {
            surface: "projects",
            path: `/projects?project=${encodeURIComponent(record.projectId)}`,
            label: "Open project",
          },
        },
        now,
...
function executionBlocker(task: ProjectExecutionRecord["tasks"][number]) {
  if (/live journey|owner-facing completion/i.test(task.safeMessage))
    return {
      code: "live_journey_incomplete",
      title: "Live journey proof required",
      nextAction:
        "Run the owner journey against the exact integrated revision, then retry completion",
    };
  if (/environment|ENOENT|npm ci|dependency/i.test(task.safeMessage))
    return {
      code: "execution_environment_unavailable",
      title: "Execution environment needs repair",
      nextAction:
        "Repair the local project environment, verify it, then resume the bounded task",
    };
  if (/provider|free route|capacity/i.test(task.safeMessage))
    return {
      code: "free_provider_unavailable",
      title: "Free provider unavailable",
      nextAction:
        "Wait for a verified free route or retry when free capacity returns",
    };
  if (task.status === "quarantined")
    return {
      code: "execution_quarantined",
      title: "Verified evidence is quarantined",
      nextAction:
        "Review the quarantined evidence and choose a bounded recovery",
    };
  return {
    code: "execution_needs_owner",
    title: "Execution needs a decision",
    nextAction: "Review the evidence and choose an available safe recovery",
  };
}

function supersededByLifecycle(
  recommendation: AutonomySnapshot["recommendations"][number],
  lifecycles: readonly ProjectLifecycleRecord[],
): boolean {
  if (
    !recommendation.projectId ||
    !["approve_request", "provide_input"].includes(recommendation.boundary)
  )
    return false;
  const lifecycle = lifecycles.find(
    (item) => item.projectId === recommendation.projectId,
  );
  return lifecycle
    ? !["intake", "context", "clarification"].includes(lifecycle.stage)
    : false;
}
...
function fromLifecycle(
  ...
  if (record.stage === "awaiting_design_approval") {
    const solution = record.artifacts.find(
      (artifact) => artifact.kind === "solution",
    );
    if (!solution) return [];
    return [
      decision(
        {
          ...
          reason:
            "Product and technical review passed. Delivery planning remains blocked until the owner decides.",
          nextAction: "Review, approve, request changes, or decline",
          authorityBoundary: "approve_solution",
          effect: "authorized_local_write",
          reversible: false,
          ...
          requestId: null,
          providerId: null,
          source: "project_solution",
          sourceRecordId: solution.digest,
          evidence: [
            `Solution revision ${solution.revision}`,
            `${solution.citations.length} cited sources`,
            `${solution.reviewerIds.length} independent reviewers`,
            `Digest: ${solution.digest.slice(0, 12)}`,
          ],
          reference: {
            surface: "decisions",
            path: `/decisions?project=${encodeURIComponent(record.projectId)}`,
  ...
  if (record.stage !== "clarification" || record.questions.length === 0)
    return [];
    ...
    decision(
      {
        ...
        evidence: [
          `${question.options.length} selectable options`,
          ...question.sourceFindingIds
            .slice(0, 8)
            .map((id) => `Finding: ${id}`),
        ],
        reference: {
          surface: "work",
          path: `/work?project=${encodeURIComponent(record.projectId)}`,
          label: "Review in Build",
        },
      },
      now,
    ),
...
function fromLiveEvent(
  event: LiveOperationsSnapshot["recentEvents"][number],
  now: number,
  lifecycles: readonly ProjectLifecycleRecord[],
): DecisionItem[] {
  if (event.projectId && terminalLifecycle(event.projectId, lifecycles))
    return [];
  const state = event.state.toLowerCase();
  const category = liveCategory(state, event.kind);
  if (!category) return [];
  const requestReference = event.requestId
    ? workReference(event.requestId)
    : null;
  const reference =
    requestReference ??
    (event.kind === "project"
      ? {
          ...
          path: `/projects${event.projectId ? `?project=${encodeURIComponent(event.projectId)}` : ""}`,
          label: "Open project",
        }
      : event.kind === "provider"
        ? {
            surface: "providers" as const,
            path: `/providers${event.providerId ? `?provider=${encodeURIComponent(event.providerId)}` : ""}`,
            label: "Open provider",
          }
        : {
    ...
    decision(
      {
        ...
        evidence: [
          "Canonical live operation",
          `${readable(event.kind)} state: ${readable(event.state)}`,
        ],
        reference,
      },
      now,
...
function terminalLifecycle(
  ...
  const lifecycle = lifecycles.find((item) => item.projectId === projectId);
  return lifecycle?.stage === "complete" || lifecycle?.stage === "cancelled";
}

function fromProvider(
  provider: LiveOperationsSnapshot["providers"][number],
  now: number,
): DecisionItem[] {
  if (provider.state === "ready" && provider.admitted && provider.zeroCost)
    return [];
  return [
    decision(
      {
        ...
        reason:
          provider.state === "revoked"
            ? "The local connection is revoked and cannot receive work."
            : provider.state === "stale"
              ? "Provider readiness evidence is stale and must be refreshed."
              : "The free provider currently has limited capacity.",
        nextAction:
          provider.state === "revoked"
        ...
        evidence: [
          ...
          `$0 route: ${provider.zeroCost ? "yes" : "no"}`,
        ],
        reference: {
          surface: "providers",
          path: `/providers?provider=${encodeURIComponent(provider.providerId)}`,
          label: "Open provider",
        },
      },
      now,
...
function fromRecommendation(
  item: AutonomySnapshot["recommendations"][number],
  now: number,
): DecisionItem[] {
  if (
    item.classification === "terminal" ||
    (item.classification === "safe_action" && item.automaticAllowed)
  )
    return [];
  const category: DecisionCategory =
    item.classification === "approval"
      ? item.boundary === "provide_input" || item.boundary === "approve_request"
        ? "input"
        : "approval"
...
function decision(
  input: Omit<DecisionItem, "id" | "age" | "maximumCostUsd"> & { seed: string },
  now: number,
): DecisionItem {
...
function liveCategory(state: string, kind: string): DecisionCategory | null {
  ...
  if (/(failed|error|interrupted|rejected|cancelled|corrupt)/.test(state))
    return "failure";
  if (/(expired|reconcile|outcome_unknown)/.test(state)) return "recovery";
  if (
    kind === "provider" &&
    /(limited|stale|revoked|unavailable|quota)/.test(state)
  )
    return "provider";
  if (
    kind === "project" &&
    /(warning|blocked|missing|denied|unavailable)/.test(state)
  )
    return "project";
  if (kind === "system" && /(conflict|duplicate)/.test(state))
    return "conflict";
...
function priorityFor(
  ...
  if (
    /(corrupt|security|credential|data_loss|outcome_unknown)/.test(state) ||
...
function boundaryFor(category: DecisionCategory): string {
  ...
  if (category === "recovery" || category === "failure")
...
function projectForRequest(
  ...
  return (
    snapshot.recommendations.find((item) => item.requestId === requestId)
      ?.projectId ??
    snapshot.overrides.find((item) => item.requestId === requestId)
      ?.projectId ??
...
function workReference(requestId: string) {
  return {
    ...
    path: `/work?request=${encodeURIComponent(requestId)}`,
    label: "Open work",
  };
}
...
function facet(items: DecisionItem[], value: (item: DecisionItem) => string) {
  ...
  for (const item of items)
    counts.set(value(item), (counts.get(value(item)) ?? 0) + 1);
  return [...counts]
    .sort(
      (left, right) => right[1] - left[1] || left[0].localeCompare(right[0]),
    )
    .map(([entry, count]) => ({ value: entry, count }));
}

function inRange(
  value: number,
  range: DecisionQuery["range"],
  now: number,
): boolean {
...
function safeText(value: string, maximum: number): string {
  ...
  return bounded.length <= maximum
    ? bounded
    : `${bounded.slice(0, maximum - 1)}…`;
}
