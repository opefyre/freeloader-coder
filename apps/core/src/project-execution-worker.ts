...
export interface ProjectExecutionAdapters {
  ...
  integrate(
    ...
    validation: WorkerValidation;
    liveJourneyEvidence?: ExecutionTask["liveJourneyEvidence"];
  }>;
...
export class ProjectExecutionWorker {
  ...
  async tick(projectId: string) {
    let record =
      (await this.service.get(projectId)) ??
      (await this.service.initialize(projectId));
    if (record.state !== "running") return record;
    const reconciledBefore = new Set(
      record.tasks
        .filter((task) => task.reconciliationEvidence)
        .map((task) => task.id),
    );
    record = await this.service.reconcileEquivalentQueued(projectId);
    const newlyReconciled = record.tasks.find(
      (task) => task.reconciliationEvidence && !reconciledBefore.has(task.id),
    );
    if (newlyReconciled)
      await this.adapters
        .observe?.(projectId, newlyReconciled)
        .catch(() => undefined);
    if (record.state !== "running") return record;
    ...
    const queued = record.tasks.find(
      (task) =>
        task.status === "queued" &&
    ...
    try {
      ...
      while (true) {
        ...
        if (!fast.passed) {
          ...
          implementation = await this.adapters.heal(
            projectId,
            task,
            task.attempt,
          );
      ...
      task = await this.service.recordReviews(
        projectId,
        task.id,
        lease.leaseId,
        this.workerId,
        await this.adapters.review(projectId, task),
      );
      ...
      task = await this.service.recordIntegration(
        projectId,
        task.id,
        lease.leaseId,
        this.workerId,
      ...
      await this.adapters.observe?.(projectId, task).catch((error) => {
        console.warn(
          "Completed execution could not be synchronized to its external observer; canonical completion proof is preserved for retry.",
          {
            projectId,
            taskId: task.id,
            error:
      ...
      const proposalContractFailure =
        error instanceof Error &&
        /Provider proposal (?:failed strict response contract|exceeded grounded file authority|conflicts with observed file state|omitted required task files)/.test(
          error.message,
        );
      if (
        error instanceof FreeProviderExecutionError &&
        ...
        if (task.attempt < policy.maxAttempts) {
          await this.service.releaseContractFailureForRetry(
            projectId,
            task.id,
            lease.leaseId,
            this.workerId,
            task.assignment.providerId,
            "The provider response failed the source contract before any workspace mutation. Codkesh rotated to another eligible free provider automatically.",
          );
        ...
        await this.service.interrupt(
          ...
          lease.leaseId,
          this.workerId,
          "Execution needs attention: Every bounded provider-output repair attempt failed the source contract before mutation.",
          "implementation",
        );
        await this.adapters.observe?.(projectId, task).catch(() => undefined);
        return await this.service.get(projectId);
        ...
        const failureClass =
          ...
          !["canonical_dirty", "repository_invalid"].includes(error.code)
  ...
  private async heal(
    ...
    const [failureClass, policy] = await Promise.all([
      this.adapters.classifyFailure(projectId, task, validation),
      this.adapters.healingPolicy(projectId, task),
    ]);
    return this.service.assessHealing(
      projectId,
      task.id,
      leaseId,
      this.workerId,
      {
        ...
        policy,
        goldenScore: implementation.goldenScore,
        previousGoldenScore: implementation.previousGoldenScore,
      },
    );
  }
}
