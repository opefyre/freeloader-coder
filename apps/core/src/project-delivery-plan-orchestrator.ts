...
function deterministicPlan(solution: SolutionContent, contextDigest: string, solutionDigest: string) {
  ...
  const scaffold = {
    ...
    implementationNotes: ["Install and pin a free local toolchain. Every script must execute real analysis or assertions and fail on a deliberately invalid fixture; unconditional-success commands are forbidden.", "For TypeScript, pin Node types, TypeScript-aware ESLint parsing, and a portable TypeScript test runner; verify from a clean checkout without parent-directory dependencies.", `Complete downstream dependency inventory: ${[...new Set(sections.flatMap(([, , requirements]) => requirements))].join(" ")}`], roleCapabilities: ["Developer", "QA reviewer"],
