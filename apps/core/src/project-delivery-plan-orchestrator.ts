...
function deterministicPlan(solution: SolutionContent, contextDigest: string, solutionDigest: string) {
  ...
  const scaffold = {
    ...
    implementationNotes: ["Install and pin a free local toolchain. Every script must execute real analysis or assertions and fail on a deliberately invalid fixture; unconditional-success commands are forbidden."], roleCapabilities: ["Developer", "QA reviewer"],
    rollbackRequirements: ["Revert the scaffold files and restore the approved pre-execution baseline."], allowedFiles: ["package.json", "package-lock.json", "tsconfig.json", "eslint.config.js", "tests/scaffold.test.ts"], validationProfiles: ["format", "lint", "typecheck", "unit"], citations: ["local://CONTEXT.md", "local://DESIGN.md"],
  };
  items.push(
  ...
  const coverage = sections.map(([requirement, title, requirements, file], index) => {
    ...
    const base = {
      ...
      allowedFiles: [file, testFile], validationProfiles: ["format", "lint", "typecheck", "unit"], citations: ["local://CONTEXT.md", "local://DESIGN.md"],
    };
    items.push(
