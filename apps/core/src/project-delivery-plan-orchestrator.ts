...
function deterministicPlan(solution: SolutionContent, contextDigest: string, solutionDigest: string) {
  const sections = [
    ["behavior", "Product behavior", solution.behavior, "src/features/core.ts"],
    ["architecture", "Local application architecture", solution.architecture, "src/app.ts"],
    ["user_experience", "Owner experience", solution.userExperience, "src/ui/app.ts"],
    ["data", "Local data model", solution.data, "src/data/store.ts"],
    ["integrations", "Integration boundaries", solution.integrations, "src/integrations/boundaries.ts"],
    ["security", "Application security", solution.security, "src/security/policy.ts"],
  ...
  const scaffold = {
    ...
    implementationNotes: ["Install and pin a free local toolchain. Every script must execute real analysis or assertions and fail on a deliberately invalid fixture; unconditional-success commands are forbidden.", "For TypeScript, pin Node types, TypeScript-aware ESLint parsing, and a portable TypeScript test runner; verify from a clean checkout without parent-directory dependencies.", "Complete downstream dependency inventory: TypeScript, @types/node, ESLint, typescript-eslint, Prettier, Vite, Vitest, Playwright, @axe-core/playwright, fake-indexeddb, browser IndexedDB APIs, service-worker caching, accessible semantic HTML, and CSS."], roleCapabilities: ["Developer", "QA reviewer"],
  ...
  const coverage = sections.map(([requirement, title, requirements, file], index) => {
    ...
    const base = {
      ...
      implementationNotes: requirements.map((note) => note.slice(0, 2_000)), roleCapabilities: ["Developer", "QA reviewer"], rollbackRequirements: [`Revert ${file} and restore the last verified behavior for this scope.`],
...
function assertNewProductBootstrap(plan: ReturnType<typeof deliveryPlanContentSchema.parse>) {
  ...
  if (!scaffold.implementationNotes.some((note) => note.startsWith("Complete downstream dependency inventory:") && note.length > "Complete downstream dependency inventory:".length + 10)) throw new Error("New-product scaffold must name the complete downstream dependency inventory before execution.");
