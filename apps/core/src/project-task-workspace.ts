...
const GENERATED_VALIDATION_DIRECTORIES = [
  "dist",
  "build",
  "coverage",
  ".vite",
...
export class ProjectTaskWorkspaceService {
    ...
    try {
      ...
      if (!info.isDirectory() || info.isSymbolicLink())
        throw new ProjectTaskWorkspaceError(
          "workspace_conflict",
          "Existing task workspace is unsafe.",
        );
      const observed = (
      ...
      if (observed !== baseline) {
        const descendant = await run(
          "git",
          ["merge-base", "--is-ancestor", baseline, observed],
          workspace,
          GIT_TIMEOUT_MS,
        ).then(
          () => true,
        ...
        if (
          !descendant ||
          [...committed, ...working].some(
            (path) => !allowed.has(path) && !path.startsWith("node_modules/"),
          )
        ) {
          throw new ProjectTaskWorkspaceError(
            "workspace_conflict",
            "Existing task workspace baseline changed outside exact file authority.",
          );
        }
      }
    } catch (error) {
    ...
    for (const path of task.allowedFiles) {
      ...
      try {
        ...
        if (!info.isFile() || info.isSymbolicLink() || info.size > 128 * 1024)
          throw new ProjectTaskWorkspaceError(
            "source_unsupported",
            `${path} is not a supported text source.`,
          );
        ...
        if (total > 768 * 1024)
          throw new ProjectTaskWorkspaceError(
            "source_unsupported",
            "Bounded task sources exceed 768 KiB.",
          );
  ...
  async resetAuthorizedFiles(
    ...
    for (const path of task.allowedFiles) {
      ...
      const tracked = await run(
        "git",
        ["cat-file", "-e", `${workspace.baseline}:${path}`],
        workspace.root,
        GIT_TIMEOUT_MS,
      ).then(
        () => true,
      ...
      if (tracked) {
        await run(
          "git",
          ["checkout", workspace.baseline, "--", path],
          workspace.root,
          GIT_TIMEOUT_MS,
        );
        continue;
      ...
      try {
        ...
        if (!info.isFile() || info.isSymbolicLink())
          throw new ProjectTaskWorkspaceError(
            "workspace_conflict",
            "Repair target is not a safe regular file.",
          );
        await unlink(target);
      } catch (error) {
        if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
  ...
  async apply(
    ...
    if (operations.length === 0 || operations.length > task.allowedFiles.length)
      throw new ProjectTaskWorkspaceError(
        "operation_denied",
        "Implementation must contain bounded file operations.",
      );
    if (
      ...
      throw new ProjectTaskWorkspaceError(
        "operation_denied",
        "Implementation contains duplicate file operations.",
      );
    const allowed = new Set(task.allowedFiles);
    const planned: Array<{
    ...
    for (const operation of operations) {
      if (
        ...
        Buffer.byteLength(operation.content) > 128 * 1024 ||
        operation.content.includes("\0")
      )
        throw new ProjectTaskWorkspaceError(
          "operation_denied",
          "Implementation exceeded exact file authority.",
        );
      const target = await approvedTarget(
        workspace.root,
        operation.path,
      ...
      if (operation.type === "create" && before !== null)
        throw new ProjectTaskWorkspaceError(
          "stale_source",
      ...
      if (
        ...
        (before === null || hash(before) !== operation.expectedBeforeDigest)
      )
        throw new ProjectTaskWorkspaceError(
          "stale_source",
          "Replace target changed after grounding.",
        );
    ...
    try {
      ...
      for (const entry of [...applied].reverse()) {
        if (entry.before === null)
          await unlink(entry.target).catch(() => undefined);
        else await atomicWrite(entry.target, entry.before, entry.mode);
      }
      throw new ProjectTaskWorkspaceError(
        "operation_denied",
        "Implementation apply failed and prior file states were restored.",
      );
    }
    const changed = parseChanged(
      await git(workspace.root, [
    ...
    if (changed.some((path) => !allowed.has(path)))
      throw new ProjectTaskWorkspaceError(
        "operation_denied",
        "Observed changes escaped exact file authority.",
      );
    return {
  ...
  async prepareDependencies(
    ...
    try {
      await lstat(resolve(workspace.root, "package.json"));
    } catch {
      return null;
    }
    let lockfileExisted = true;
    try {
      await lstat(resolve(workspace.root, "package-lock.json"));
    } catch {
      lockfileExisted = false;
    }
    const canUpdateLockfile =
      task.allowedFiles.includes("package.json") &&
      task.allowedFiles.includes("package-lock.json");
    if (!lockfileExisted && !canUpdateLockfile) {
      throw new ProjectTaskWorkspaceError(
        "validation_unavailable",
        "Dependency installation requires a reviewed lockfile or exact scaffold authority.",
      );
    }
    ...
    const allowed = new Set(task.allowedFiles);
    if (
      (!lockfileExisted && !changed.includes("package-lock.json")) ||
      changed.some((path) => !allowed.has(path))
    )
      throw new ProjectTaskWorkspaceError(
        "operation_denied",
        "Dependency preparation changed files outside exact scaffold authority.",
      );
    return {
      changedFiles: changed,
      evidenceDigest: hash(
        JSON.stringify({
          authorityDigest: workspace.authorityDigest,
          command: "npm-install-lock-and-ci-ignore-scripts",
          changed,
        }),
      ),
    };
  ...
  async formatAuthorizedFiles(
    ...
    const files = (
      await existingAllowedFiles(workspace.root, task.allowedFiles)
    ).filter(isPrettierSupportedFile);
    if (files.length === 0) return null;
    await run(
      resolve(workspace.root, "node_modules", ".bin", "prettier"),
      ["--write", ...files],
      workspace.root,
      VALIDATION_TIMEOUT_MS,
    );
    ...
    if (changed.some((path) => !allowed.has(path)))
      throw new ProjectTaskWorkspaceError(
        "operation_denied",
        "Deterministic formatting changed files outside exact task authority.",
      );
    return {
      changedFiles: changed,
      evidenceDigest: hash(
        JSON.stringify({
          authorityDigest: workspace.authorityDigest,
          command: "prettier-write-authorized-files",
          changed,
        }),
      ),
    };
  ...
  async validate(
    ...
    if (
      task.allowedFiles.includes("package.json") &&
      task.validationProfiles.includes("unit")
    ) {
      const testCommand = manifest.scripts?.[scripts.unit];
      if (testCommand && !isProjectWideTestCommand(testCommand)) {
        throw new ProjectTaskWorkspaceError(
          "validation_unavailable",
          "A new-product test command must discover the complete test suite, not only the scaffold test.",
        );
      }
    }
    ...
    for (const profile of task.validationProfiles) {
      ...
      if (!manifest.scripts?.[script])
        throw new ProjectTaskWorkspaceError(
          "validation_unavailable",
          `Validation profile ${profile} is not configured by the project.`,
        );
      if (isNoOpValidationScript(manifest.scripts[script])) {
        throw new ProjectTaskWorkspaceError(
          "validation_unavailable",
          `Validation profile ${profile} is configured with a no-op command and cannot produce trustworthy evidence.`,
        );
      }
      try {
        const command = manifest.scripts[script];
        const result =
          profile === "format" && isPrettierCheck(command)
            ? await run(
                resolve(root, "node_modules", ".bin", "prettier"),
                [
                  "--check",
                  ...(
                    await existingAllowedFiles(root, task.allowedFiles)
                  ).filter(isPrettierSupportedFile),
                ],
                root,
                VALIDATION_TIMEOUT_MS,
            ...
            : profile === "typecheck" && isScopedTypeScriptCheck(command)
              ? await run(
                  ...
                  ["--noEmit", "--typeRoots", "./node_modules/@types"],
                  root,
                  VALIDATION_TIMEOUT_MS,
                )
              : profile === "unit"
        ...
        results.push({
          ...
          exitCode: 0,
          output,
          evidenceDigest: hash(`${profile}:0:${output}`),
        });
      } catch (error) {
        ...
        results.push({
          ...
          output,
          evidenceDigest: hash(`${profile}:1:${output}`),
        });
        break;
      }
    }
  ...
  async validateCommit(
    ...
    try {
      ...
      const workspace: PreparedTaskWorkspace = {
        projectId: "project_0000000000000000",
        taskId: task.id,
        root: checkout,
        branch: "detached",
        baseline: commitDigest,
        authorityDigest: hash(`clean-validation:${commitDigest}`),
      };
  ...
  async commit(workspace: PreparedTaskWorkspace, task: ExecutionTask) {
    ...
    if (changed.length === 0) {
      const existing = await existingAllowedFiles(
        workspace.root,
        task.allowedFiles,
      );
      if (
      ...
      return {
        ...
        evidenceDigest: hash(
          JSON.stringify({
            ...
            changed,
            authorityDigest: workspace.authorityDigest,
            revalidatedExistingDelivery: true,
          }),
        ),
      };
    }
    if (changed.some((path) => !task.allowedFiles.includes(path)))
      throw new ProjectTaskWorkspaceError(
        "operation_denied",
        "Commit changes do not match exact file authority.",
      );
    ...
    await git(workspace.root, [
      ...
      "user.name=Codkesh",
      "-c",
      "user.email=codkesh@localhost",
      "commit",
      "--no-verify",
      "-m",
      `${task.jiraIssueKey}: ${task.title}`,
    ]);
    const commitDigest = (
      await git(workspace.root, ["rev-parse", "--verify", "HEAD"])
    ).trim();
  ...
  async integrate(
    ...
    if (
      ...
      throw new ProjectTaskWorkspaceError(
        "canonical_dirty",
        "Canonical project changed before integration.",
      );
    if (
      ...
      throw new ProjectTaskWorkspaceError(
        ...
        "Canonical baseline changed before integration.",
      );
    await git(root, ["merge", "--ff-only", commitDigest]);
    const observed = (
      await git(root, ["rev-parse", "--verify", "HEAD"])
    ).trim();
    if (observed !== commitDigest)
      throw new ProjectTaskWorkspaceError(
        "integration_conflict",
        "Integrated commit could not be verified.",
      );
    return {
      integrationDigest: hash(
        JSON.stringify({
  ...
  async revertIntegration(
    ...
    if (
      ...
      throw new ProjectTaskWorkspaceError(
        "integration_conflict",
        "Automatic integration restore was refused because canonical Git changed.",
      );
    await git(root, [
      "-c",
      "user.name=Codkesh",
...
async function removeGeneratedValidationArtifacts(root: string) {
  for (const directory of GENERATED_VALIDATION_DIRECTORIES) {
    ...
    const tracked = await run(
      "git",
      ["ls-files", "--error-unmatch", "--", directory],
      root,
      GIT_TIMEOUT_MS,
    ).then(
      () => true,
    ...
    try {
      ...
      if (!info.isDirectory() || info.isSymbolicLink())
        throw new ProjectTaskWorkspaceError(
          "workspace_conflict",
          `Generated validation path ${directory} is unsafe.`,
        );
...
function isNoOpValidationScript(command: string) {
  ...
  return (
    ...
    (/console\.log\s*\(/.test(normalized) &&
      !/[;&|]\s*(?:node|npm|npx|tsc|eslint|prettier|vitest|jest)\b/.test(
        normalized,
      ))
  );
}
...
function isPrettierCheck(command: string) {
  return /(?:^|\s)prettier\s+--check(?:\s|$)/.test(
...
async function existingAllowedFiles(
  ...
  if (existing.length === 0)
    throw new ProjectTaskWorkspaceError(
      "validation_unavailable",
      "No authorized files are available for formatting validation.",
    );
  return existing;
}

function isPrettierSupportedFile(path: string) {
  return /\.(?:[cm]?[jt]sx?|json|md|mdx|css|scss|less|html|ya?ml|graphql|gql)$/i.test(
    path,
  );
}
...
async function runUnitValidation(root: string, task: ExecutionTask) {
  ...
  const declaredTests = task.allowedFiles.filter((path) =>
    /(^|\/)(?:tests?|__tests__)\/.*\.(?:[cm]?[jt]sx?)$|\.(?:test|spec)\.[cm]?[jt]sx?$/.test(
      path,
    ),
  );
  if (declaredTests.length === 0) return projectResult;
  const taskTests = await existingAllowedFiles(root, declaredTests).catch(
    (): string[] => [],
  );
  if (taskTests.length !== declaredTests.length) {
    ...
    throw new ProjectTaskWorkspaceError(
      "validation_unavailable",
      `Task-owned unit tests are missing: ${missing.join(", ")}.`,
    );
  }
  const usesTypeScript = taskTests.some((path) => /\.[cm]?tsx?$/.test(path));
  const args = usesTypeScript
    ? ["--import", "tsx", "--test", ...taskTests]
    : ["--test", ...taskTests];
...
export class ProjectTaskWorkspaceError extends Error {
  constructor(
    readonly code:
      | "repository_invalid"
      | "canonical_dirty"
...
async function assertRepository(root: string) {
  ...
  if (top !== root)
    throw new ProjectTaskWorkspaceError(
      "repository_invalid",
      "Selected folder is not the canonical Git repository root.",
    );
}
async function ensureExecutionBaseline(root: string) {
  ...
  if (!existing && changed.length === 0)
    throw new ProjectTaskWorkspaceError(
      "repository_invalid",
      "The new project has no approved files for its first Git checkpoint.",
    );
  if (changed.some((path) => !isCodkeshOwnedBaselinePath(path))) {
    throw new ProjectTaskWorkspaceError(
      "canonical_dirty",
      "The project contains changes Codkesh does not own. Review and checkpoint them before autonomous implementation.",
    );
  }
  ...
  await git(root, [
    ...
    "-m",
    existing
      ? "chore: checkpoint approved Codkesh plan"
      : "chore: save approved Codkesh baseline",
  ]);
  ...
  if (
    ...
    throw new ProjectTaskWorkspaceError(
      "repository_invalid",
      "Codkesh could not verify the new project's first Git checkpoint.",
    );
  }
  return baseline;
}
async function verifyGovernedArtifactMirrors(
  root: string,
  changed: readonly string[],
) {
  const topLevelArtifacts = changed.filter((path) =>
    GOVERNED_ARTIFACTS.has(path),
  );
  for (const path of topLevelArtifacts) {
    const historyRoot = join(root, ".codkesh", "artifacts", basename(path));
    let versions: string[];
    try {
      versions = (await readdir(historyRoot))
        .filter((entry) => entry.endsWith(".md"))
        .sort()
        .map((entry) => join(historyRoot, entry));
    } catch {
      throw new ProjectTaskWorkspaceError(
        "canonical_dirty",
        `${path} has no immutable Codkesh artifact evidence.`,
      );
    }
    ...
    if (
      ...
      hash(await readFile(join(root, path), "utf8")) !==
        hash(await readFile(latest, "utf8"))
    ) {
      throw new ProjectTaskWorkspaceError(
        "canonical_dirty",
        `${path} differs from its latest immutable Codkesh artifact evidence.`,
      );
    }
  }
}
...
function isCodkeshOwnedBaselinePath(path: string) {
  return (
    ...
    path.startsWith(".codkesh/artifacts/")
  );
}
const GOVERNED_ARTIFACTS = new Set([
  "CONTEXT.md",
  "DECISIONS.md",
  "DELIVERY-PLAN.md",
  "DESIGN.md",
  ...
  for (const component of parentRelation.split(sep).filter(Boolean)) {
    ...
    try {
      ...
      if (!info.isDirectory() || info.isSymbolicLink())
        throw new ProjectTaskWorkspaceError(
          "operation_denied",
          "Task parent contains an unsafe path component.",
        );
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
...
function assertWithin(root: string, target: string, allowRoot = false) {
  ...
  if (
    ...
    throw new ProjectTaskWorkspaceError(
      "operation_denied",
      "Path escaped the isolated workspace.",
    );
}
function parseChanged(status: string) {
  if (!status) return [];
  const entries = status.split("\0").filter(Boolean);
  const paths: string[] = [];
  for (let index = 0; index < entries.length; index += 1) {
    const code = entries[index]!.slice(0, 2);
    let path = entries[index]!.slice(3);
    if (code.includes("R")) {
...
async function git(cwd: string, args: readonly string[]) {
  try {
    return (await run("git", args, cwd, GIT_TIMEOUT_MS)).stdout;
  } catch (error) {
    throw new ProjectTaskWorkspaceError(
      "repository_invalid",
      error instanceof Error
        ? error.message.replaceAll(cwd, "<repository>").slice(0, 300)
...
async function atomicWrite(path: string, content: string, mode: number) {
  const temporary = `${path}.${process.pid}.${hash(path).slice(0, 8)}.studio.tmp`;
  try {
