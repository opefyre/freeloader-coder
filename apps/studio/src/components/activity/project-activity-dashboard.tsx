      ...
      {status === "offline" ? (
        ...
        actionItems.length === 0 && solutionItems.length === 0 && clarificationItems.length === 0 && researchActions.length === 0 && backlogActions.length === 0 && executionActions.length === 0 ? (
          ...
          <div className="grid gap-3">
            {clarificationItems.map((lifecycle) => <button key={`clarification-${lifecycle.projectId}`} type="button" onClick={() => { setSelectedClarification(lifecycle); setClarificationChoices({}); setCustomAnswers({}); setNotice(""); }} className="rounded-3xl bg-card p-5 text-left outline-none hover:bg-muted/55 focus-visible:ring-3 focus-visible:ring-ring/30"><div className="flex items-start gap-4"><span className="grid size-10 shrink-0 place-items-center rounded-2xl bg-amber-500/12 text-amber-500"><Warning weight="fill" /></span><span className="min-w-0 flex-1"><span className="flex flex-wrap items-center gap-2"><strong>Choose the project direction</strong><Badge>{projectNames.get(lifecycle.projectId) ?? "Unknown project"}</Badge></span><span className="mt-2 block text-sm leading-6 text-muted-foreground">{lifecycle.questions.length} focused question{lifecycle.questions.length === 1 ? "" : "s"} block the next stage. Current work remains safe.</span></span><span className="text-xs text-muted-foreground">Answer</span></div></button>)}
