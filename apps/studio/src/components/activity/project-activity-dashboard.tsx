...
export function ProjectActivityDashboard(props: {
  ...
  const [lifecycles, setLifecycles] = useState<
    readonly ProjectLifecycleRecord[]
  >([]);
  ...
  const [executions, setExecutions] = useState<
    readonly ProjectExecutionRecord[]
  >([]);
  const [selectedSolution, setSelectedSolution] =
    useState<ProjectLifecycleRecord | null>(null);
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setLifecycles(
        ...
        ).filter((record): record is ProjectLifecycleRecord => record !== null),
      );
      setSolutionRuns(
        (
          await Promise.all(
        ...
        ).filter((record): record is SolutionRun => record !== null),
      );
      setBacklogRuns(
        (
          await Promise.all(
        ...
        ).filter((record): record is DeliveryPlanRun => record !== null),
      );
      setExecutions(
        (
          await Promise.all(
        ...
        ).filter((record): record is ProjectExecutionRecord => record !== null),
      );
      setStatus("live");
    } catch {
      setStatus("offline");
  ...
  const actionItems = visibleRequests.filter(
    (request) =>
      request.state === "needs_input" ||
      request.state === "interrupted" ||
      (request.plan?.state === "draft" && request.grounding),
  );
  const solutionItems = lifecycles.filter(
    (lifecycle) =>
      lifecycle.stage === "awaiting_design_approval" &&
      (projectId === "all" || lifecycle.projectId === projectId),
  );
  const clarificationItems = lifecycles.filter(
    (lifecycle) =>
      lifecycle.stage === "clarification" &&
      lifecycle.questions.length > 0 &&
      (projectId === "all" || lifecycle.projectId === projectId),
  );
  const researchActions = solutionRuns.filter(
    (run) =>
      run.state === "needs_user" &&
      (projectId === "all" || run.projectId === projectId),
  );
  const backlogActions = backlogRuns.filter(
    (run) =>
      run.state === "needs_user" &&
      (projectId === "all" || run.projectId === projectId),
  );
  ...
  async function decide(
    ...
    if (decision === "revision_requested" && feedback.trim().length < 3) {
    ...
    try {
      await decideProjectSolution({
        ...
        expectedRevision: selectedSolution.revision,
        artifactDigest: solution.digest,
        decision,
        feedback: decision === "revision_requested" ? feedback.trim() : null,
  ...
  async function answerClarifications() {
    ...
    const answers: OwnerAnswer[] = selectedClarification.questions.map(
      (question) => {
        ...
        return choice === "__custom__"
          ? {
              ...
              optionId: null,
              customAnswer: customAnswers[question.id]?.trim() || null,
              answeredAt: Date.now(),
            }
          : {
              questionId: question.id,
              optionId: choice ?? null,
              customAnswer: null,
              answeredAt: Date.now(),
            };
      },
    ...
    if (
      answers.some(
        (answer) => answer.optionId === null && answer.customAnswer === null,
      )
    ) {
      setNotice("Answer every question to continue.");
      return;
    }
    setWorking(true);
    setNotice("");
    try {
      await answerProjectClarifications({
        endpoint: props.endpoint,
        projectId: selectedClarification.projectId,
        expectedRevision: selectedClarification.revision,
        answers,
      ...
      setNotice(
        error instanceof Error
          ? error.message
          : "The answers could not be saved safely.",
      );
    } finally {
      setWorking(false);
    }
    ...
    <section className="space-y-5">
      ...
      {status === "offline" ? (
        <Empty
          ...
          detail="No cached or sample status is substituted."
        />
      ) : props.mode === "actions" ? (
        ...
        executionActions.length === 0 ? (
          <Empty
            ...
            detail="The pipeline can continue without an owner decision right now."
            positive
          />
        ) : (
        ...
        <div className="space-y-3">
          ...
          >
            ...
            />
            <Metric
              value={`${completedProjects}/${scopedProjects.length}`}
              label="Complete"
            />
          </div>
          <div className="grid gap-3 lg:grid-cols-2">
            {scopedProjects.map((project) => {
              const jira = (project.resources ?? []).find(
              ...
              const verified =
                execution?.tasks.filter((task) => task.status === "completed")
                  .length ?? 0;
              const projectAttention =
                actionItems.filter((item) => item.projectId === project.id)
                  .length +
                solutionItems.filter((item) => item.projectId === project.id)
                  .length +
                clarificationItems.filter(
                  (item) => item.projectId === project.id,
                ).length +
                researchActions.filter((item) => item.projectId === project.id)
                  .length +
                backlogActions.filter((item) => item.projectId === project.id)
                  .length +
                executionActions.filter((item) => item.projectId === project.id)
                  .length;
              const providers = new Set(
                execution?.tasks.flatMap((task) =>
                  [
                    task.assignment?.providerId,
                    ...task.reviews.map((review) => review.providerId),
                  ].filter((value): value is string => Boolean(value)),
                ) ?? [],
              );
              const validations =
                execution?.tasks.flatMap((task) => task.validations) ?? [];
              const passedValidations = validations.filter(
                (item) => item.passed,
              ).length;
              const retries =
                execution?.tasks.reduce((sum, task) => sum + task.attempt, 0) ??
                0;
              const cycleSamples =
                execution?.tasks.flatMap((task) =>
                  task.assignment
                    ? [Math.max(0, task.updatedAt - task.assignment.selectedAt)]
                    : [],
                ) ?? [];
              const cycleTime =
                cycleSamples.length > 0
                  ? formatDuration(median(cycleSamples))
                  : "Unknown";
              const stale = project.progress
                ? Date.now() - project.progress.observedAt > 5 * 60_000
                : false;
              return (
                <Card key={project.id}>
                  <CardContent className="p-5">
                      ...
                      {jira && project.progress ? (
                        <>
                          ...
                          <div
                            ...
                            role="img"
                            aria-label={`${project.progress.percent}% complete: ${project.progress.completed} of ${project.progress.total} Jira items completed, ${project.progress.blocked} blocked`}
                          >
                            <div
                          ...
                          <p className="mt-3 text-xs text-muted-foreground">
                            {project.progress.completed} of{" "}
                            {project.progress.total} completed ·{" "}
                            {project.progress.blocked} blocked ·{" "}
                            {stale
                              ? "stale"
                              : formatFreshness(project.progress.observedAt)}
                          </p>
                        </>
                      ) : (
                        <div className="flex items-start gap-3">
                          <Warning className="mt-0.5 shrink-0 text-amber-500" />
                          <div>
                            <strong className="text-sm">
                              Jira progress unavailable
                            </strong>
                            <p className="mt-1 text-xs leading-5 text-muted-foreground">
                              {jira
                                ? "The selected Jira project has not completed its first synchronization."
                                : "Select a Jira project in this project's setup to make Jira the progress source."}
                            </p>
                          </div>
                        </div>
                      )}
                    ...
                    <div className="mt-4">
                      ...
                      {project.latestUpdate?.url ? (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="mt-2 flex items-start gap-2 text-sm leading-6 hover:text-primary"
                        >
                      ...
                      <p className="mt-2 text-xs text-muted-foreground">
                        {project.latestUpdate
                          ? `${project.latestUpdate.source} · ${formatFreshness(project.latestUpdate.occurredAt)}`
                          : "Unknown · no source observed"}
                      </p>
                    </div>
                    <div
                      className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-5"
                      aria-label={`${project.displayName} delivery health from canonical local execution evidence`}
                    >
                      <MiniMetric
                        value={`${verified}/${execution?.tasks.length ?? 0}`}
                        label="Tasks"
                      />
                      <MiniMetric
                    ...
                    <a
                      ...
                      className="mt-4 inline-flex min-h-10 items-center text-sm font-medium text-primary outline-none hover:underline focus-visible:ring-3 focus-visible:ring-ring/30"
                    >
                      Open project files
                    </a>
                  </CardContent>
          ...
          {jiraProjects === 0 && scopedProjects.length > 0 && (
            <p className="text-center text-xs text-muted-foreground">
              No selected Jira project has produced a current observation.
            </p>
          )}
        </div>
      )}
          ...
          <Card className="w-full max-w-xl">
            <CardContent className="p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <Badge>{projectNames.get(selected.projectId)}</Badge>
                  <h2
              ...
              {selected.findings.length > 0 && (
                <ul className="mt-4 space-y-2">
                  {selected.findings.map((finding) => (
                    <li
                      key={finding.code}
                      className="rounded-2xl bg-muted p-3 text-xs leading-5"
                    >
      ...
      {selectedSolution && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="solution-review-title"
        >
          <Card className="max-h-[90vh] w-full max-w-3xl overflow-auto">
            <CardContent className="p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <Badge>{projectNames.get(selectedSolution.projectId)}</Badge>
                  <h2
          ...
          <Card className="max-h-[90vh] w-full max-w-2xl overflow-auto">
            <CardContent className="p-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <Badge>
                    {projectNames.get(selectedClarification.projectId)}
                  </Badge>
                  <h2
              ...
              <div className="mt-5 space-y-5">
                {selectedClarification.questions.map((question) => (
                  <fieldset
                    key={question.id}
                    className="rounded-3xl bg-muted/45 p-4"
                  >
                    ...
                    <div className="mt-3 grid gap-2 sm:grid-cols-2">
                      {question.options.map((option) => (
                        <button
                          ...
                          className={`rounded-2xl px-4 py-3 text-left text-sm font-semibold ${clarificationChoices[question.id] === option.id ? "bg-primary text-primary-foreground" : "bg-background hover:bg-background/70"}`}
                        >
                          {option.label}
                          {question.recommendedOptionId === option.id
                            ...
                            : ""}
                          <span
                            className={`mt-1 block text-xs font-normal ${clarificationChoices[question.id] === option.id ? "text-primary-foreground/75" : "text-muted-foreground"}`}
                          >
                            {option.consequence}
                          </span>
                        </button>
                      ...
                      {question.allowsCustomAnswer && (
                        <button
                          ...
                          className={`rounded-2xl px-4 py-3 text-left text-sm font-semibold ${clarificationChoices[question.id] === "__custom__" ? "bg-primary text-primary-foreground" : "bg-background hover:bg-background/70"}`}
                        >
                          Something else
                        </button>
                      )}
                    ...
                    {clarificationChoices[question.id] === "__custom__" && (
                      <textarea
                        ...
                        rows={2}
                        maxLength={2_000}
                        className="mt-3 w-full resize-y rounded-2xl bg-background px-4 py-3 text-sm outline-none focus-visible:ring-3 focus-visible:ring-ring/30"
                      />
                    )}
                  </fieldset>
                ))}
              ...
              {notice && (
                <p role="status" className="mt-4 text-xs text-amber-500">
                  {notice}
                </p>
              )}
              <div className="mt-5 flex justify-end">
                <Button
                  ...
                  disabled={
                    working ||
                    selectedClarification.questions.some(
...
function actionTitle(request: LocalRequest): string {
  if (request.state === "interrupted")
    return "Interrupted work needs a decision";
...
function Empty(props: { title: string; detail: string; positive?: boolean }) {
  return (
    <div className="grid min-h-64 place-items-center rounded-4xl bg-muted/45 p-8 text-center">
      <div>
        {props.positive ? (
          <CheckCircle
          ...
          <Warning className="mx-auto text-amber-500" size={30} weight="fill" />
        )}
...
function Metric(props: {
  ...
  return (
    <div className="rounded-3xl bg-muted/45 p-5">
      <strong
        className={
          props.tone === "attention" ? "text-2xl text-amber-500" : "text-2xl"
        }
      >
        {props.value}
      </strong>
