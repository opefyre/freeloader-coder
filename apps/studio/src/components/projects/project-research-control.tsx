...
import { generateProjectBacklog, generateProjectSolution, getProjectBacklogRun, getProjectExecution, getProjectLifecycle, getProjectProviderConsent, getProjectSolutionRun } from "../../local-project-client.js";
...
export function ProjectResearchControl(props: { endpoint: string; projectId: string }) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      const connected = providerCollection.connections.filter((provider) => provider.credentialState === "active" && provider.cost.zeroCost && !provider.cost.billingEnabled);
      setLifecycle(nextLifecycle); setProviders(connected); setConsent(nextConsent); setRun(nextRun); setBacklogRun(nextBacklogRun); setExecution(nextExecution);
  ...
  async function startResearch() {
    ...
    try {
      setRun(await generateProjectSolution({ endpoint: props.endpoint, projectId: props.projectId, idempotencyKey: `solution-generate:${crypto.randomUUID()}` })); await refresh();
  ...
  return <><div className="mt-4 rounded-2xl bg-muted/55 p-4">
    ...
    {providers.length === 0 ? <p className="mt-3 flex items-center gap-2 text-xs text-muted-foreground"><Warning className="text-amber-500" />No connected free provider is available yet.</p> : <>
      <p className="mt-3 text-xs leading-5 text-muted-foreground">All connected free providers are included automatically. Codkesh refreshes availability, chooses the best route, and never uses paid capacity.</p>
      {validConsent && <div className="mt-3 flex flex-wrap gap-2">{validConsent.providerIds.map((providerId) => <Badge key={providerId}>{providers.find((provider) => provider.providerId === providerId)?.providerLabel ?? providerId}</Badge>)}</div>}
      <Button className="mt-3 w-full" size="sm" disabled={working || Boolean(active) || !contextDigest} onClick={() => void startResearch()}><Play weight="fill" />{active ? "Research in progress" : "Start research"}</Button>
    </>}
