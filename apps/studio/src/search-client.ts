import {
  searchQuerySchema,
  universalSearchSnapshotSchema,
  type SearchQuery,
  type UniversalSearchSnapshot,
} from "../../../packages/runtime/src/universal-search.js";

const MAX_RESPONSE_BYTES = 700_000;

export async function fetchUniversalSearch(input: {
  endpoint: string;
  query?: Partial<SearchQuery>;
  signal?: AbortSignal;
  fetcher?: typeof fetch;
}): Promise<UniversalSearchSnapshot> {
  const endpoint = validateEndpoint(input.endpoint);
  const query = searchQuerySchema.parse(input.query ?? {});
  const url = new URL("/api/v1/search", endpoint);
  if (query.query) url.searchParams.set("q", query.query);
  query.scopes.forEach((value) => url.searchParams.append("scope", value));
  url.searchParams.set("limit", String(query.limit));
  const response = await (input.fetcher ?? fetch)(url, {
    method: "GET",
    cache: "no-store",
    credentials: "omit",
    headers: { Accept: "application/json" },
    ...(input.signal ? { signal: input.signal } : {}),
  });
  if (!response.ok) throw new Error(response.status === 400 ? "Search query is invalid." : "Local search is unavailable.");
  const declared = Number(response.headers.get("content-length") ?? 0);
  if (declared > MAX_RESPONSE_BYTES) throw new Error("Search response is too large.");
  const text = await response.text();
  if (text.length > MAX_RESPONSE_BYTES) throw new Error("Search response is too large.");
  return universalSearchSnapshotSchema.parse(JSON.parse(text) as unknown);
}

function validateEndpoint(value: string): URL {
  const endpoint = new URL(value);
  if (
    endpoint.protocol !== "http:" ||
    !["127.0.0.1", "localhost", "::1"].includes(endpoint.hostname) ||
    endpoint.username ||
    endpoint.password ||
    endpoint.pathname !== "/" ||
    endpoint.search ||
    endpoint.hash
  ) throw new Error("Control-plane endpoint must be an origin-only loopback HTTP URL.");
  return endpoint;
}
