...
const faq = [
  ["Is this another code editor?", "No. Codkesh is a control and evidence layer around repositories, models, isolated execution, validation, and human decisions."],
...
function ThemeButton() {
  ...
  const toggle = () => {
    ...
    window.localStorage.setItem("codkesh-site-theme", next ? "dark" : "light");
  };
...
function App() {
  ...
  return <>
    ...
    <header className="site-header"><a className="brand" href="#top" aria-label="Codkesh home"><img src="/pipeline-studio-mark.svg" alt="Codkesh logo" /><span><strong>Codkesh</strong><small>Autonomous product studio</small></span></a><nav className={menu ? "site-nav open" : "site-nav"} aria-label="Primary"><a href="#demo">Demo</a><a href="#capabilities">How it works</a><a href="#trust">Trust</a><a href="#compare">Compare</a><a href="#faq">FAQ</a></nav><div className="header-actions"><ThemeButton /><a className="button secondary desktop-action" href={repositoryUrl} target="_blank" rel="noreferrer"><GithubLogo size={18} /> GitHub</a><button className="icon-button menu-button" type="button" aria-expanded={menu} aria-label={menu ? "Close menu" : "Open menu"} onClick={() => setMenu((value) => !value)}>{menu ? <X size={20} /> : <List size={20} />}</button></div></header>
    <main id="main">
      <section className="hero section" id="top"><div className="hero-copy"><a className="status-chip" href="#trust"><span /> Public preview live · field evidence pending <ArrowRight size={15} /></a><p className="eyebrow">Autonomous development with visible proof</p><h1>Build with AI.<br /><em>Keep control.</em></h1><p className="hero-lede">Codkesh turns a request into grounded, isolated, validated code changes—while you keep authority over cost, integration, and publishing.</p><div className="hero-actions"><a className="button primary" href={repositoryUrl} target="_blank" rel="noreferrer"><GithubLogo size={19} weight="fill" /> View source <ArrowUpRight size={17} /></a><a className="button secondary" href="#demo"><Play size={17} weight="fill" /> Explore the demo</a></div><div className="hero-proof"><span><CheckCircle weight="fill" /> Full test suite passing</span><span><LockKey weight="fill" /> $0 automatic spend</span><span><ShieldCheck weight="fill" /> Evidence before Done</span></div></div>
        <div className="hero-visual" aria-label="Illustrative Codkesh evidence flow"><div className="visual-top"><span className="window-dots"><i /><i /><i /></span><span>Illustrative run · no live effect</span><span className="verified"><CheckCircle weight="fill" /> Bounded demo</span></div><div className="visual-request"><span>Your request</span><strong>Add a trustworthy project activity feed</strong></div><div className="visual-flow">{["Grounded plan","Isolated change","Validation","Independent review"].map((item,index)=><div key={item} className="flow-step"><span>{String(index+1).padStart(2,"0")}</span><strong>{item}</strong><CheckCircle size={18} weight="fill" /></div>)}</div><div className="visual-result"><Sparkle size={23} weight="fill" /><div><span>Ready for your decision</span><strong>Preview evidence · local checks · $0.00</strong></div><ArrowRight size={19} /></div></div>
      </section>
      ...
      <Demo />
      <section className="section trust-section" id="trust"><div className="trust-intro"><p className="eyebrow">Trust is a product surface</p><h2>Every important claim carries its boundary.</h2><p>Codkesh does not turn roadmap intent into present-tense marketing. Verified, bounded, and unavailable are deliberately different states.</p><div className="trust-count"><strong>{verifiedCount}</strong><span>foundational claims verified against repository evidence</span></div></div><div className="claim-list">{publicClaims.map((claim)=><a className="claim" key={claim.id} href={claim.source} target="_blank" rel="noreferrer"><span className={`claim-status ${claim.status}`}>{claim.status}</span><div><strong>{claim.label}</strong><p>{claim.detail}</p><small>{claim.sourceLabel} <ArrowUpRight size={12} /></small></div></a>)}</div></section>
